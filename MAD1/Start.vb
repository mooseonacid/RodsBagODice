...
Public Class Start
    Private Sub Start_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        EncounterPCNumControl.SelectedIndex = 3
        EncounterLevelControl.SelectedIndex = 2
        EncounterDiffControl.SelectedItem = "Any"
        EncounterEnvControl.SelectedItem = "Any"
    End Sub
    ...
    'encounter gen button
    Private Sub encounterGenButton_Click(sender As Object, e As EventArgs) Handles EncounterGenButton.Click
        If EncounterPCNumControl.SelectedItem IsNot Nothing Then
            Dim partySize As Integer = CType(EncounterPCNumControl.SelectedItem, Integer)
            Dim partyLevel As Integer = CType(EncounterLevelControl.SelectedItem, Integer)
            Dim environments As String = CType(EncounterEnvControl.SelectedItem, String)

            historybox.AppendText("===== RANDOM ENCOUNTER =====" & Environment.NewLine)
            For i As Integer = 1 To 10
                Dim totalXP As Integer = 0

                Dim difficulty As String = CType(EncounterDiffControl.SelectedItem, String)
                If difficulty = "Any" Then
                    Dim difficulties As String() = {"Easy", "Medium", "Hard", "Deadly"}
                    Dim random As New Random()
                    difficulty = difficulties(random.Next(difficulties.Length))
                End If
                Dim encounter As List(Of Creature) = GenerateRandomEncounter(partySize, partyLevel, difficulty, environments)

                ' Group the creatures by their name
                Dim groupedCreatures = encounter.GroupBy(Function(c) c.Name)
                Dim creatureStrings As New List(Of String)

                historybox.AppendText(i & ". ")
                For Each group In groupedCreatures
                    Dim groupXP = group.Sum(Function(c) c.XP)
                    totalXP += groupXP  ' Add group's XP to total XP

                    Dim groupString As String
                    If group.Count() > 1 Then
                        groupString = group.Count().ToString() & " x " & group.Key & " (CR: " & group.First().Challenge.ToString() & "); "
                    Else
                        groupString = group.Key & " (CR: " & group.First().Challenge.ToString() & "); "
                    End If
                    creatureStrings.Add(groupString)
                Next
                Dim encounterString As String = String.Join("and ", creatureStrings)

                historybox.AppendText(encounterString & difficulty & ", " & totalXP.ToString() & " xp" & Environment.NewLine)
                If i < 10 Then
                    historybox.AppendText(Environment.NewLine)
                End If
                historybox.ScrollToCaret()
    ...
    'Gen encounter
    Public Function GenerateRandomEncounter(partySize As Integer, partyLevel As Integer, difficulty As String, environment As String) As List(Of Creature)
        Dim totalCR As Double = CalculateTotalCR(partySize, partyLevel, difficulty)

        ' Define the CR ranges for each difficulty level
        Dim difficultyRanges As New Dictionary(Of String, Tuple(Of Double, Double)) From {
            {"Easy", Tuple.Create(0.0, 2.0)},
            {"Medium", Tuple.Create(2.0, 7.0)},
            {"Hard", Tuple.Create(7.0, 12.0)},
            {"Deadly", Tuple.Create(12.0, 18.0)}
        }

        ' Get the CR range for the selected difficulty level
        Dim crRange As Tuple(Of Double, Double) = difficultyRanges(difficulty)

        ' Filter the creatures based on the CR range and the environment
        Dim filteredCreatures As List(Of Creature) = CreatureDatabase.Creatures.Where(Function(c) c.Challenge >= crRange.Item1 And c.Challenge <= crRange.Item2 And (c.Environments.Contains(environment) Or environment = "Any")).ToList()

        ' Group the creatures by type
        Dim creatureGroups As List(Of IGrouping(Of String, Creature)) = filteredCreatures.GroupBy(Function(c) c.Type).ToList()

        ' Select a random group of similar creatures
        Dim random As New Random()
        Dim creatureGroup As IGrouping(Of String, Creature) = creatureGroups(random.Next(creatureGroups.Count))

        ' Select random creatures from the group until the total CR is close to the total CR for the party
        Dim selectedCreatures As New List(Of Creature)
        Dim totalSelectedCR As Double = 0
        Dim creaturesInGroup As List(Of Creature) = creatureGroup.ToList()
        While totalSelectedCR < totalCR And creaturesInGroup.Count > 0
            ' Select a random creature
            Dim creature As Creature = creaturesInGroup(random.Next(creaturesInGroup.Count))

            ' Make lower CR creatures more likely to be selected
            If creature.Challenge <= 1 OrElse random.NextDouble() < creature.Challenge / totalCR Then
                If totalSelectedCR + creature.Challenge <= totalCR Then
                    selectedCreatures.Add(creature)
                    totalSelectedCR += creature.Challenge
                Else
                    Exit While
                End If
            End If
        ...
        ' If no creatures have been added, add the first creature from the group
        If selectedCreatures.Count = 0 And creaturesInGroup.Count > 0 Then
            selectedCreatures.Add(creaturesInGroup(0))
        End If
    ...
    'Get party CR
    Private Function CalculateTotalCR(partySize As Integer, partyLevel As Integer, difficulty As String) As Double
        ' XP Thresholds per Character for each difficulty at each level (1-20)
        Dim xpThresholds As New Dictionary(Of String, Integer()) From {
            {"Easy", New Integer() {25, 50, 75, 125, 250, 300, 350, 450, 550, 600, 800, 1000, 1100, 1250, 1400, 1600, 2000, 2100, 2400, 2800}},
            {"Medium", New Integer() {50, 100, 150, 250, 500, 600, 750, 900, 1100, 1200, 1600, 2000, 2200, 2500, 2800, 3200, 3900, 4100, 4800, 5700}},
            {"Hard", New Integer() {75, 150, 225, 375, 750, 900, 1100, 1400, 1600, 1900, 2400, 3000, 3400, 3800, 4300, 4800, 5900, 6200, 7200, 8500}},
            {"Deadly", New Integer() {100, 200, 300, 500, 1100, 1200, 1500, 1800, 2100, 2400, 3200, 4000, 4500, 5100, 5700, 6400, 7900, 8300, 9600, 11300}}
        }

        ' XP to CR conversions / ???
        Dim xpToCR As New Dictionary(Of Integer, Double) From {
            {25, 0.125}, {50, 0.25}, {100, 0.5}, {200, 1}, {450, 2}, {700, 3}, {1100, 4}, {1800, 5}, {2300, 6}, {2900, 7}, {3900, 8}, {5000, 9}, {5900, 10}, {7200, 11}, {8400, 12}, {10000, 13}, {11500, 14}, {13000, 15}, {15000, 16}, {18000, 17}, {20000, 18}, {22000, 19}, {25000, 20}, {33000, 21}, {41000, 22}, {50000, 23}, {62000, 24}, {74000, 25}, {90000, 26}, {105000, 27}, {120000, 28}, {135000, 29}, {155000, 30}
        }

        ' Get the XP Threshold per Character for the desired difficulty and party level
        Dim xpThresholdPerCharacter As Integer = xpThresholds(difficulty)(partyLevel - 1)

        ' Calculate the total XP Threshold for the party
        Dim totalXPThreshold As Integer = xpThresholdPerCharacter * partySize

        ' Convert the total XP Threshold to a total CR
        Dim totalCR As Double = 0
        For Each xp In xpToCR.Keys.OrderByDescending(Function(x) x)
            If totalXPThreshold >= xp Then
                totalCR = xpToCR(xp)
                Exit For
            End If
        Next
...
Public Class Creature
    ...
    Public Property Source As String
    Public Property Environments As List(Of String)

    Public Sub New(name As String, size As String, type As String, alignment As String, challenge As Double, xp As Integer, source As String, environments As List(Of String))
        Me.Name = name
        Me.Size = size
        Me.Type = type
...
Public Module CreatureDatabase
    Public ReadOnly Creatures As New List(Of Creature) From {
        New Creature("Frog", "Tiny", "Beast", "Unaligned", 0, 0, "mm322", New List(Of String) From {"fff", "fff"}),
        New Creature("Sea Horse", "Tiny", "Beast", "Unaligned", 0, 0, "mm337", New List(Of String) From {"fff", "fff"}),
        New Creature("Awakened Shrub", "Small", "Plant", "Unaligned", 0, 10, "mm317", New List(Of String) From {"Forest", ""}),
        New Creature("Baboon", "Small", "Beast", "Unaligned", 0, 10, "mm318", New List(Of String) From {"Forest", ""}),
        New Creature("Badger", "Tiny", "Beast", "Unaligned", 0, 10, "mm318", New List(Of String) From {"Forest", ""}),
        New Creature("Bat", "Tiny", "Beast", "Unaligned", 0, 10, "mm318", New List(Of String) From {"fff", ""}),
        New Creature("Cat", "Tiny", "Beast", "Unaligned", 0, 10, "mm 320", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Commoner", "Medium", "Humanoid", "Any", 0, 10, "mm 344", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Crab", "Tiny", "Beast", "Unaligned", 0, 10, "mm 320", New List(Of String) From {"Coastal", ""}),
        New Creature("Cranium Rat", "Tiny", "Beast", "LE", 0, 10, "motm 83, vgm 133", New List(Of String) From {"fff", ""}),
        New Creature("Crawling Claw", "Tiny", "Undead", "NE", 0, 10, "mm 44", New List(Of String) From {"fff", ""}),
        New Creature("Deer", "Medium", "Beast", "Unaligned", 0, 10, "mm 321", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Eagle", "Small", "Beast", "Unaligned", 0, 10, "mm 322", New List(Of String) From {"Coastal", "Grassland"}),
        New Creature("Giant Fire Beetle", "Small", "Beast", "Unaligned", 0, 10, "mm 325", New List(Of String) From {"fff", ""}),
        New Creature("Goat", "Medium", "Beast", "Unaligned", 0, 10, "mm 330", New List(Of String) From {"Grassland", ""}),
        New Creature("Hawk", "Tiny", "Beast", "Unaligned", 0, 10, "mm 330", New List(Of String) From {"fff", ""}),
        New Creature("Homunculus", "Tiny", "Construct", "N", 0, 10, "mm 188", New List(Of String) From {"fff", ""}),
        New Creature("Hyena", "Medium", "Beast", "Unaligned", 0, 10, "mm 331", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Jackal", "Small", "Beast", "Unaligned", 0, 10, "mm 331", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Lemure", "Medium", "Fiend (devil)", "LE", 0, 10, "mm 76", New List(Of String) From {"fff", ""}),
        New Creature("Lizard", "Tiny", "Beast", "Unaligned", 0, 10, "mm 332", New List(Of String) From {"fff", ""}),
        New Creature("Myconid Sprout", "Small", "Plant", "LN", 0, 10, "mm 230", New List(Of String) From {"fff", ""}),
        New Creature("Octopus", "Small", "Beast", "Unaligned", 0, 10, "mm 333", New List(Of String) From {"fff", ""}),
        New Creature("Owl", "Tiny", "Beast", "Unaligned", 0, 10, "mm 333", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Quipper", "Tiny", "Beast", "Unaligned", 0, 10, "mm 335", New List(Of String) From {"fff", ""}),
        New Creature("Rat", "Tiny", "Beast", "Unaligned", 0, 10, "mm 335", New List(Of String) From {"fff", ""}),
        New Creature("Raven", "Tiny", "Beast", "Unaligned", 0, 10, "mm 335", New List(Of String) From {"fff", ""}),
        New Creature("Scorpion", "Tiny", "Beast", "Unaligned", 0, 10, "mm 337", New List(Of String) From {"Desert", ""}),
        New Creature("Shrieker", "Medium", "Beast", "Unaligned", 0, 10, "mm 138", New List(Of String) From {"fff", ""}),
        New Creature("Spider", "Tiny", "Beast", "Unaligned", 0, 10, "mm 337", New List(Of String) From {"fff", ""}),
        New Creature("Vulture", "Medium", "Beast", "Unaligned", 0, 10, "mm 339", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Weasel", "Tiny", "Beast", "Unaligned", 0, 10, "mm 340", New List(Of String) From {"fff", ""}),
        New Creature("Bandit", "Medium", "Humanoid", "Any non-lawful", 0.125, 25, "mm 343", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Blood Hawk", "Small", "Beast", "Unaligned", 0.125, 25, "mm 319", New List(Of String) From {"Arctic", "Coastal", "Forest", "Grassland"}),
        New Creature("Boggle", "Small", "Fey", "CN", 0.125, 25, "motm 65, vgm 128", New List(Of String) From {"Forest", ""}),
        New Creature("Camel", "Large", "Beast", "Unaligned", 0.125, 25, "mm 320", New List(Of String) From {"Desert", ""}),
        New Creature("Cultist", "Medium", "Humanoid", "Any non-good", 0.125, 25, "mm 345", New List(Of String) From {"fff", ""}),
        New Creature("Dolphin", "Medium", "Beast", "Unaligned", 0.125, 25, "motm 97, vgm 208", New List(Of String) From {"Coastal", ""}),
        New Creature("Flumph", "Small", "Aberration", "LG", 0.125, 25, "mm 135", New List(Of String) From {"fff", ""}),
        New Creature("Flying Snake", "Tiny", "Beast", "Unaligned", 0.125, 25, "mm 322", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Giant Crab", "Medium", "Beast", "Unaligned", 0.125, 25, "mm 324", New List(Of String) From {"Coastal", ""}),
        New Creature("Giant Rat", "Small", "Beast", "Unaligned", 0.125, 25, "mm 327", New List(Of String) From {"Forest", ""}),
        New Creature("Giant Weasel", "Medium", "Beast", "Unaligned", 0.125, 25, "mm 329", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Guard", "Medium", "Humanoid", "Any", 0.125, 25, "mm 347", New List(Of String) From {"Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Kobold", "Small", "Humanoid (kobold)", "LE", 0.125, 25, "mm 195", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Manes", "Small", "Fiend (demon)", "CE", 0.125, 25, "mm 60", New List(Of String) From {"fff", ""}),
        New Creature("Mastiff", "Medium", "Beast", "Unaligned", 0.125, 25, "mm 332", New List(Of String) From {"Forest", ""}),
        New Creature("Merfolk", "Medium", "Humanoid (merfolk)", "N", 0.125, 25, "mm 218", New List(Of String) From {"Coastal", ""}),
        New Creature("Monodrone", "Medium", "Construct", "LN", 0.125, 25, "mm 224", New List(Of String) From {"fff", ""}),
        New Creature("Mule", "Medium", "Beast", "Unaligned", 0.125, 25, "mm 333", New List(Of String) From {"Desert", ""}),
        New Creature("Neogi Hatchling", "Tiny", "Aberration", "LE", 0.125, 25, "motm 191, vgm 179", New List(Of String) From {"fff", ""}),
        New Creature("Noble", "Medium", "Humanoid", "Any", 0.125, 25, "mm 348", New List(Of String) From {"fff", ""}),
        New Creature("Poisonous Snake", "Tiny", "Beast", "Unaligned", 0.125, 25, "mm 334", New List(Of String) From {"Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Pony", "Medium", "Beast", "Unaligned", 0.125, 25, "mm 335", New List(Of String) From {"fff", ""}),
        New Creature("Slaad Tadpole", "Tiny", "Aberration", "CN", 0.125, 25, "mm 276", New List(Of String) From {"fff", ""}),
        New Creature("Stirge", "Tiny", "Beast", "Unaligned", 0.125, 25, "mm 284", New List(Of String) From {"Coastal", "Desert", "Forest"}),
        New Creature("Tribal Warrior", "Medium", "Humanoid", "Any", 0.125, 25, "mm 350", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Twig Blight", "Small", "Plant", "NE", 0.125, 25, "mm 32", New List(Of String) From {"Forest", ""}),
        New Creature("Xvart", "Small", "Humanoid (xvart)", "CE", 0.125, 25, "motm 267, vgm 200", New List(Of String) From {"fff", ""}),
        New Creature("Young Kruthik", "Small", "Monstrosity", "Unaligned", 0.125, 25, "motm 168, mtf 211", New List(Of String) From {"Desert", ""}),
        New Creature("Aarakocra", "Medium", "Humanoid (aarakocra)", "NG", 0.25, 50, "mm 12", New List(Of String) From {"fff", ""}),
        New Creature("Abyssal Wretch", "Medium", "Fiend (demon)", "CE", 0.25, 50, "mtf 136", New List(Of String) From {"fff", ""}),
        New Creature("Acolyte", "Medium", "Humanoid", "Any", 0.25, 50, "mm 342", New List(Of String) From {"", ""}),
        New Creature("Apprentice Wizard", "Medium", "Humanoid", "Any", 0.25, 50, "motm 259, vgm 209", New List(Of String) From {"", ""}),
        New Creature("Axe Beak", "Large", "Beast", "Unaligned", 0.25, 50, "mm 317", New List(Of String) From {"", ""}),
        New Creature("Blink Dog", "Medium", "Fey", "LG", 0.25, 50, "mm 318", New List(Of String) From {"Forest", ""}),
        New Creature("Boar", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 319", New List(Of String) From {"Forest", ""}),
        New Creature("Bullywug", "Medium", "Humanoid (bullywug)", "NE", 0.25, 50, "mm 35", New List(Of String) From {"", ""}),
        New Creature("Constrictor Snake", "Large", "Beast", "Unaligned", 0.25, 50, "mm 320", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Cow", "Large", "Beast", "Unaligned", 0.25, 50, "vgm 207", New List(Of String) From {"", ""}),
        New Creature("Deep Rothe", "Medium", "Beast", "Unaligned", 0.25, 50, "motm 71, vgm 207", New List(Of String) From {"", ""}),
        New Creature("Derro", "Small", "Humanoid (derro)", "CE", 0.25, 50, "motm 91, mtf 158", New List(Of String) From {"", ""}),
        New Creature("Dimetrodon", "Medium", "Beast", "Unaligned", 0.25, 50, "motm 95, vgm 139", New List(Of String) From {"Coastal", ""}),
        New Creature("Draft Horse", "Large", "Beast", "Unaligned", 0.25, 50, "mm 321", New List(Of String) From {"", ""}),
        New Creature("Dretch", "Small", "Fiend (demon)", "CE", 0.25, 50, "mm 57", New List(Of String) From {"", ""}),
        New Creature("Drow", "Medium", "Humanoid (elf)", "NE", 0.25, 50, "mm 128", New List(Of String) From {"", ""}),
        New Creature("Duodrone", "Medium", "Construct", "LN", 0.25, 50, "mm 225", New List(Of String) From {"", ""}),
        New Creature("Elk", "Large", "Beast", "Unaligned", 0.25, 50, "mm 322", New List(Of String) From {"Forest", ""}),
        New Creature("Flying Sword", "Small", "Construct", "Unaligned", 0.25, 50, "mm 20", New List(Of String) From {"", ""}),
        New Creature("Giant Badger", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 323", New List(Of String) From {"Forest", ""}),
        New Creature("Giant Bat", "Large", "Beast", "Unaligned", 0.25, 50, "mm 323", New List(Of String) From {"Forest", ""}),
        New Creature("Giant Centipede", "Small", "Beast", "Unaligned", 0.25, 50, "mm 323", New List(Of String) From {"", ""}),
        New Creature("Giant Frog", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 325", New List(Of String) From {"Forest", ""}),
        New Creature("Giant Lizard", "Large", "Beast", "Unaligned", 0.25, 50, "mm 326", New List(Of String) From {"Coastal", "Desert", "Forest"}),
        New Creature("Giant Owl", "Large", "Beast", "N", 0.25, 50, "mm 327", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Giant Poisonous Snake", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 327", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Giant Wolf Spider", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 330", New List(Of String) From {"Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Gnoll Witherling", "Medium", "Undead", "CE", 0.25, 50, "motm 145, vgm 155", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Goblin", "Small", "Humanoid (goblinoid)", "NE", 0.25, 50, "mm 166", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Grimlock", "Medium", "Humanoid (grimlock)", "NE", 0.25, 50, "mm 175", New List(Of String) From {"", ""}),
        New Creature("Grung", "Small", "Humanoid (grung)", "LE", 0.25, 50, "motm 149, vgm 156", New List(Of String) From {"Forest", ""}),
        New Creature("Hadrosaurus", "Large", "Beast", "Unaligned", 0.25, 50, "motm 96, vgm 140", New List(Of String) From {"Grassland", ""}),
        New Creature("Kenku", "Medium", "Humanoid (kenku)", "CN", 0.25, 50, "mm 194", New List(Of String) From {"Forest", ""}),
        New Creature("Kobold Inventor", "Small", "Humanoid (kobold)", "LE", 0.25, 50, "motm 164, vgm 166", New List(Of String) From {"Forest", ""}),
        New Creature("Kuo-Toa", "Medium", "Humanoid (kuo-toa)", "NE", 0.25, 50, "mm 199", New List(Of String) From {"", ""}),
        New Creature("Male Steeder", "Medium", "Monstrosity", "Unaligned", 0.25, 50, "motm 231, mtf 238", New List(Of String) From {"", ""}),
        New Creature("Mud Mephit", "Small", "Elemental", "NE", 0.25, 50, "mm 216", New List(Of String) From {"", ""}),
        New Creature("Needle Blight", "Medium", "Plant", "NE", 0.25, 50, "mm 32", New List(Of String) From {"Forest", ""}),
        New Creature("Oblex Spawn", "Tiny", "Ooze", "LE", 0.25, 50, "motm 197, mtf 217", New List(Of String) From {"", ""}),
        New Creature("Ox", "Large", "Beast", "Unaligned", 0.25, 50, "motm 72, vgm 207", New List(Of String) From {"", ""}),
        New Creature("Panther", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 333", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Pixie", "Tiny", "Fey", "NG", 0.25, 50, "mm 253", New List(Of String) From {"Forest", ""}),
        New Creature("Pseudodragon", "Tiny", "Dragon", "NG", 0.25, 50, "mm 254", New List(Of String) From {"Coastal", "Desert", "Forest"}),
        New Creature("Pteranodon", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 80", New List(Of String) From {"Coastal", "Grassland"}),
        New Creature("Riding Horse", "Large", "Beast", "Unaligned", 0.25, 50, "mm 336", New List(Of String) From {"Grassland", ""}),
        New Creature("Rothe", "Large", "Beast", "Unaligned", 0.25, 50, "vgm 207", New List(Of String) From {"Grassland", ""}),
        New Creature("Skeleton", "Medium", "Undead", "LE", 0.25, 50, "mm 272", New List(Of String) From {"", ""}),
        New Creature("Smoke Mephit", "Small", "Elemental", "NE", 0.25, 50, "mm 217", New List(Of String) From {"", ""}),
        New Creature("Sprite", "Tiny", "Fey", "NG", 0.25, 50, "mm 283", New List(Of String) From {"Forest", ""}),
        New Creature("Star Spawn Grue", "Small", "Aberration", "NE", 0.25, 50, "motm 227, mtf 234", New List(Of String) From {"", ""}),
        New Creature("Steam Mephit", "Small", "Elemental", "NE", 0.25, 50, "mm 217", New List(Of String) From {"", ""}),
        New Creature("Stench Kow", "Large", "Beast", "Unaligned", 0.25, 50, "motm 72, vgm 207", New List(Of String) From {"", ""}),
        New Creature("Swarm of Bats", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 337", New List(Of String) From {"", ""}),
        New Creature("Swarm of Rats", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 339", New List(Of String) From {"", ""}),
        New Creature("Swarm of Ravens", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 339", New List(Of String) From {"Forest", ""}),
        New Creature("Tortle", "Medium", "Humanoid (tortle)", "LG", 0.25, 50, "motm 244, mtf 242", New List(Of String) From {"Coastal", ""}),
        New Creature("Troglodyte", "Medium", "Humanoid (troglodyte)", "CE", 0.25, 50, "mm 290", New List(Of String) From {"", ""}),
        New Creature("Vegepygmy", "Small", "Plant", "N", 0.25, 50, "motm 252, vgm 196", New List(Of String) From {"Forest", ""}),
        New Creature("Velociraptor", "Tiny", "Beast", "Unaligned", 0.25, 50, "motm 96, vgm 140", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Violet Fungus", "Medium", "Plant", "Unaligned", 0.25, 50, "mm 138", New List(Of String) From {"", ""}),
        New Creature("Winged Kobold", "Small", "Humanoid (kobold)", "LE", 0.25, 50, "mm 195", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Wolf", "Medium", "Beast", "Unaligned", 0.25, 50, "mm 341", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Wretched Sorrowsworn", "Small", "Monstrosity", "NE", 0.25, 50, "motm 224, mtf 233", New List(Of String) From {"", ""}),
        New Creature("Zombie", "Medium", "Undead", "NE", 0.25, 50, "mm 316", New List(Of String) From {"", ""}),
        New Creature("Ape", "Medium", "Beast", "Unaligned", 0.5, 100, "mm 317", New List(Of String) From {"Forest", ""}),
        New Creature("Black Bear", "Medium", "Beast", "Unaligned", 0.5, 100, "mm 318", New List(Of String) From {"Forest", ""}),
        New Creature("Chitine", "Small", "Monstrosity", "CE", 0.5, 100, "motm 75, vgm 131", New List(Of String) From {"", ""}),
        New Creature("Cockatrice", "Small", "Monstrosity", "Unaligned", 0.5, 100, "mm 42", New List(Of String) From {"Grassland", ""}),
        New Creature("Crocodile", "Large", "Beast", "Unaligned", 0.5, 100, "mm 320", New List(Of String) From {"", ""}),
        New Creature("Darkling", "Small", "Fey", "CN", 0.5, 100, "motm 84, vgm 134", New List(Of String) From {"Forest", ""}),
        New Creature("Darkmantle", "Small", "Monstrosity", "Unaligned", 0.5, 100, "mm 46", New List(Of String) From {"", ""}),
        New Creature("Dust Mephit", "Small", "Elemental", "NE", 0.5, 100, "mm 215", New List(Of String) From {"Desert", ""}),
        New Creature("Firenewt Warrior", "Medium", "Humanoid (firenewt)", "NE", 0.5, 100, "motm 125, vgm 142", New List(Of String) From {"Desert", ""}),
        New Creature("Gas Spore", "Large", "Plant", "Unaligned", 0.5, 100, "mm 138", New List(Of String) From {"", ""}),
        New Creature("Gazer", "Tiny", "Aberration", "NE", 0.5, 100, "motm 134, vgm 126", New List(Of String) From {"", ""}),
        New Creature("Giant Goat", "Large", "Beast", "Unaligned", 0.5, 100, "mm 326", New List(Of String) From {"Grassland", ""}),
        New Creature("Giant Sea Horse", "Large", "Beast", "Unaligned", 0.5, 100, "mm 328", New List(Of String) From {"", ""}),
        New Creature("Giant Wasp", "Medium", "Beast", "Unaligned", 0.5, 100, "mm 329", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Gnoll", "Medium", "Humanoid (gnoll)", "CE", 0.5, 100, "mm 163", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Gnoll Hunter", "Medium", "Humanoid (gnoll)", "CE", 0.5, 100, "motm 144, vgm 154", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Gray Ooze", "Medium", "Ooze", "Unaligned", 0.5, 100, "mm 243", New List(Of String) From {"", ""}),
        New Creature("Hobgoblin", "Medium", "Humanoid (goblinoid)", "LE", 0.5, 100, "mm 186", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Ice Mephit", "Small", "Elemental", "NE", 0.5, 100, "mm 215", New List(Of String) From {"Arctic", ""}),
        New Creature("Jackalwere", "Medium", "Humanoid (shapechanger)", "CE", 0.5, 100, "mm 193", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Lizardfolk", "Medium", "Humanoid (lizardfolk)", "N", 0.5, 100, "mm 204", New List(Of String) From {"Forest", ""}),
        New Creature("Magma Mephit", "Small", "Elemental", "NE", 0.5, 100, "mm 216", New List(Of String) From {"", ""}),
        New Creature("Magmin", "Small", "Elemental", "CN", 0.5, 100, "mm 212", New List(Of String) From {"", ""}),
        New Creature("Myconid Adult", "Medium", "Plant", "LN", 0.5, 100, "mm 232", New List(Of String) From {"", ""}),
        New Creature("Nupperibo", "Medium", "Fiend (devil)", "LE", 0.5, 100, "motm 196, mtf 168", New List(Of String) From {"", ""}),
        New Creature("Orc", "Medium", "Humanoid (orc)", "CE", 0.5, 100, "mm 246", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Orc Nurtured One of Yurtrus", "Medium", "Humanoid (orc)", "CE", 0.5, 100, "vgm 184", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Piercer", "Medium", "Monstrosity", "Unaligned", 0.5, 100, "mm 252", New List(Of String) From {"", ""}),
        New Creature("Reef Shark", "Medium", "Beast", "Unaligned", 0.5, 100, "mm 336", New List(Of String) From {"", ""}),
        New Creature("Rust Monster", "Medium", "Monstrosity", "Unaligned", 0.5, 100, "mm 262", New List(Of String) From {"", ""}),
        New Creature("Sahuagin", "Medium", "Humanoid (sahuagin)", "LE", 0.5, 100, "mm 263", New List(Of String) From {"Coastal", ""}),
        New Creature("Satyr", "Medium", "Fey", "CN", 0.5, 100, "mm 267", New List(Of String) From {"Forest", ""}),
        New Creature("Scout", "Medium", "Humanoid", "Any", 0.5, 100, "mm 349", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Shadow", "Medium", "Undead", "CE", 0.5, 100, "mm 269", New List(Of String) From {"", ""}),
        New Creature("Skulk", "Medium", "Humanoid", "CN", 0.5, 100, "motm 219, mtf 227", New List(Of String) From {"Coastal", "Forest"}),
        New Creature("Svirfneblin", "Small", "Humanoid (gnome)", "NG", 0.5, 100, "mm 164", New List(Of String) From {"", ""}),
        New Creature("Swarm of Insects", "Medium", "Beast", "Unaligned", 0.5, 100, "mm 338", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Swarm of Rot Grubs", "Medium", "Beast", "Unaligned", 0.5, 100, "motm 237, vgm 208", New List(Of String) From {"", ""}),
        New Creature("Thug", "Medium", "Humanoid", "Any", 0.5, 100, "mm 350", New List(Of String) From {"", ""}),
        New Creature("Tridrone", "Medium", "Construct", "LN", 0.5, 100, "mm 225", New List(Of String) From {"", ""}),
        New Creature("Vine Blight", "Medium", "Plant", "NE", 0.5, 100, "mm 32", New List(Of String) From {"Forest", ""}),
        New Creature("Warhorse", "Large", "Beast", "Unaligned", 0.5, 100, "mm 340", New List(Of String) From {"", ""}),
        New Creature("Warhorse Skeleton", "Large", "Undead", "LE", 0.5, 100, "mm 273", New List(Of String) From {"", ""}),
        New Creature("Worg", "Large", "Monstrosity", "NE", 0.5, 100, "mm 341", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Animated Armor", "Medium", "Construct", "Unaligned", 1, 200, "mm 19", New List(Of String) From {"", ""}),
        New Creature("Brass Dragon Wyrmling", "Medium", "Dragon", "CG", 1, 200, "mm 106", New List(Of String) From {"", ""}),
        New Creature("Bronze Scout", "Medium", "Construct", "Unaligned", 1, 200, "motm 79, mtf 125", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Brown Bear", "Large", "Beast", "Unaligned", 1, 200, "mm 319", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Bugbear", "Medium", "Humanoid (goblinoid)", "CE", 1, 200, "mm 33", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Choker", "Small", "Aberration", "CE", 1, 200, "motm 76, mtf 123", New List(Of String) From {"Forest", ""}),
        New Creature("Copper Dragon Wyrmling", "Medium", "Dragon", "CG", 1, 200, "mm 112", New List(Of String) From {"", ""}),
        New Creature("Death Dog", "Medium", "Monstrosity", "NE", 1, 200, "mm 321", New List(Of String) From {"Desert", ""}),
        New Creature("Deinonychus", "Medium", "Beast", "Unaligned", 1, 200, "motm 95, vgm 139", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Dire Wolf", "Large", "Beast", "Unaligned", 1, 200, "mm 321", New List(Of String) From {"Forest", ""}),
        New Creature("Dryad", "Medium", "Fey", "N", 1, 200, "mm 121", New List(Of String) From {"Forest", ""}),
        New Creature("Duergar", "Medium", "Humanoid (dwarf)", "LE", 1, 200, "mm 122", New List(Of String) From {"", ""}),
        New Creature("Duergar Soulblade", "Medium", "Humanoid (dwarf)", "LE", 1, 200, "motm 109, mtf 190", New List(Of String) From {"", ""}),
        New Creature("Female Steeder", "Large", "Monstrosity", "Unaligned", 1, 200, "motm 231, mtf 238", New List(Of String) From {"", ""}),
        New Creature("Fire Snake", "Medium", "Elemental", "NE", 1, 200, "mm 265", New List(Of String) From {"", ""}),
        New Creature("Firenewt Warlock of Imix", "Medium", "Humanoid (firenewt)", "NE", 1, 200, "motm 125, vgm 143", New List(Of String) From {"Desert", ""}),
        New Creature("Ghoul", "Medium", "Undead", "CE", 1, 200, "mm 148", New List(Of String) From {"", ""}),
        New Creature("Giant Eagle", "Large", "Beast", "NG", 1, 200, "mm 324", New List(Of String) From {"Coastal", "Grassland"}),
        New Creature("Giant Hyena", "Large", "Beast", "Unaligned", 1, 200, "mm 326", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Giant Octopus", "Large", "Beast", "Unaligned", 1, 200, "mm 326", New List(Of String) From {"", ""}),
        New Creature("Giant Spider", "Large", "Beast", "Unaligned", 1, 200, "mm 328", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Giant Strider", "Large", "Monstrosity", "NE", 1, 200, "motm 137, vgm 143", New List(Of String) From {"", ""}),
        New Creature("Giant Toad", "Large", "Beast", "Unaligned", 1, 200, "mm 329", New List(Of String) From {"Coastal", "Desert", "Forest"}),
        New Creature("Giant Vulture", "Large", "Beast", "NE", 1, 200, "mm 329", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Gnoll Flesh Gnawer", "Medium", "Humanoid (gnoll)", "CE", 1, 200, "motm 144, vgm 154", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Goblin Boss", "Small", "Humanoid (goblinoid)", "NE", 1, 200, "mm 166", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Grung Wildling", "Small", "Humanoid (grung)", "LE", 1, 200, "motm 150, vgm 157", New List(Of String) From {"Forest", ""}),
        New Creature("Half-ogre", "Large", "Giant", "Any chaotic", 1, 200, "mm 238", New List(Of String) From {"Arctic", "Desert", "Forest"}),
        New Creature("Harpy", "Medium", "Monstrosity", "CE", 1, 200, "mm 181", New List(Of String) From {"Coastal", "Forest"}),
        New Creature("Hippogriff", "Large", "Monstrosity", "Unaligned", 1, 200, "mm 184", New List(Of String) From {"Grassland", ""}),
        New Creature("Imp", "Tiny", "Fiend (devil,shapechanger)", "LE", 1, 200, "mm 76", New List(Of String) From {"", ""}),
        New Creature("Kobold Dragonshield", "Small", "Humanoid (kobold)", "LE", 1, 200, "motm 163, vgm 165", New List(Of String) From {"Forest", ""}),
        New Creature("Kobold Scale Sorcerer", "Small", "Humanoid (kobold)", "LE", 1, 200, "motm 165, vgm 167", New List(Of String) From {"Forest", ""}),
        New Creature("Kuo-Toa Whip", "Medium", "Humanoid (kuo-toa)", "NE", 1, 200, "mm 200", New List(Of String) From {"", ""}),
        New Creature("Lion", "Large", "Beast", "Unaligned", 1, 200, "mm 331", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Maw Demon", "Medium", "Fiend (demon)", "CE", 1, 200, "motm 176, vgm 137", New List(Of String) From {"", ""}),
        New Creature("Meazel", "Medium", "Humanoid (meazel)", "NE", 1, 200, "motm 177, mtf 214", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Nilbog", "Small", "Humanoid (goblinoid)", "CE", 1, 200, "motm 195, vgm 182", New List(Of String) From {"Forest", ""}),
        New Creature("Quadrone", "Medium", "Construct", "LN", 1, 200, "mm 226", New List(Of String) From {"", ""}),
        New Creature("Quaggoth Spore Servant", "Medium", "Plant", "Unaligned", 1, 200, "mm 230", New List(Of String) From {"", ""}),
        New Creature("Quasit", "Tiny", "Fiend (demon,shapechanger)", "CE", 1, 200, "mm 63", New List(Of String) From {"", ""}),
        New Creature("Quickling", "Tiny", "Fey", "CE", 1, 200, "motm 207. vgm 187", New List(Of String) From {"Forest", ""}),
        New Creature("Scarecrow", "Medium", "Construct", "CE", 1, 200, "mm 268", New List(Of String) From {"Grassland", ""}),
        New Creature("Sea Spawn", "Medium", "Humanoid", "NE", 1, 200, "motm 211, vgm 189", New List(Of String) From {"Coastal", ""}),
        New Creature("Specter", "Medium", "Undead", "CE", 1, 200, "mm 279", New List(Of String) From {"", ""}),
        New Creature("Spy", "Medium", "Humanoid", "Any", 1, 200, "mm 349", New List(Of String) From {"", ""}),
        New Creature("Stone Cursed", "Medium", "Construct", "LE", 1, 200, "motm 233, mtf 240", New List(Of String) From {"Desert", ""}),
        New Creature("Swarm of Quippers", "Medium", "Beast", "Unaligned", 1, 200, "mm 338", New List(Of String) From {"", ""}),
        New Creature("Thorny Vgepygmy", "Medium", "Plant", "N", 1, 200, "motm 253, vgm 197", New List(Of String) From {"Forest", ""}),
        New Creature("Thri-Kreen", "Medium", "Humanoid (thri-kreen)", "CN", 1, 200, "mm 288", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Tiger", "Large", "Beast", "Unaligned", 1, 200, "mm 339", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Vargouille", "Tiny", "Fiend", "CE", 1, 200, "motm 251, vgm 195", New List(Of String) From {"Desert", ""}),
        New Creature("Xvart Warlock of Raxivort", "Small", "Humanoid (xvart)", "CE", 1, 200, "motm 267, vgm 200", New List(Of String) From {"", ""}),
        New Creature("Young Faerie Dragon", "Tiny", "Dragon", "CG", 1, 200, "mm 133", New List(Of String) From {"Forest", ""}),
        New Creature("Yuan-Ti Pureblood", "Medium", "Humanoid (yuan-ti)", "NE", 1, 200, "mm 310", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Adult Kruthik", "Medium", "Monstrosity", "Unaligned", 2, 450, "motm 169, mtf 212", New List(Of String) From {"Desert", ""}),
        New Creature("Allosaurus", "Large", "Beast", "Unaligned", 2, 450, "mm 79", New List(Of String) From {"Grassland", ""}),
        New Creature("Ankheg", "Large", "Monstrosity", "Unaligned", 2, 450, "mm 21", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Aurochs", "Large", "Beast", "Unaligned", 2, 450, "motm 71, vgm 207", New List(Of String) From {"Grassland", ""}),
        New Creature("Awakened Tree", "Huge", "Plant", "Unaligned", 2, 450, "mm 317", New List(Of String) From {"Forest", ""}),
        New Creature("Azer", "Medium", "Elementl", "LN", 2, 450, "mm 22", New List(Of String) From {"", ""}),
        New Creature("Bandit Captain", "Medium", "Humanoid", "Any non-lawful", 2, 450, "mm 344", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Bard", "Medium", "Humanoid", "Any", 2, 450, "motm 59, vgm 211", New List(Of String) From {"", ""}),
        New Creature("Berbalang", "Medium", "Aberration", "NE", 2, 450, "motm 61, mtf 120", New List(Of String) From {"Desert", ""}),
        New Creature("Berserker", "Medium", "Humanoid", "Any chaotic", 2, 450, "mm 344", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest"}),
        New Creature("Black Dragon Wyrmling", "Medium", "Dragon", "CE", 2, 450, "mm 88", New List(Of String) From {"", ""}),
        New Creature("Bronze Dragon Wyrmling", "Medium", "Dragon", "LG", 2, 450, "mm 109", New List(Of String) From {"", ""}),
        New Creature("Carrion Crawler", "Large", "Monstrosity", "Unaligned", 2, 450, "mm 37", New List(Of String) From {"", ""}),
        New Creature("Cave Bear", "Large", "Beast", "Unaligned", 2, 450, "mm 334", New List(Of String) From {"", ""}),
        New Creature("Centaur", "Large", "Monstrosity", "NG", 2, 450, "mm 38", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Cult Fanatic", "Medium", "Humanoid", "Any", 2, 450, "mm 345", New List(Of String) From {"", ""}),
        New Creature("Darkling Elder", "Medium", "Fey", "CN", 2, 450, "motm 84, vgm 134", New List(Of String) From {"Forest", ""}),
        New Creature("Druid", "Medium", "Humanoid", "Any", 2, 450, "mm 346", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Duergar Hammerer", "Medium", "Construct", "LE", 2, 450, "motm 112, mtf 188", New List(Of String) From {"", ""}),
        New Creature("Duergar Kavalrachni", "Medium", "Humanoid (dwarf)", "LE", 2, 450, "motm 107, mtf 189", New List(Of String) From {"", ""}),
        New Creature("Duergar Mind Master", "Medium", "Humanoid (dwarf)", "LE", 2, 450, "motm 108, mtf 189", New List(Of String) From {"", ""}),
        New Creature("Duergar Stone Guard", "Medium", "Humanoid (dwarf)", "LE", 2, 450, "motm 110, mtf 191", New List(Of String) From {"", ""}),
        New Creature("Duergar Xarrorn", "Medium", "Humanoid (dwarf)", "LE", 2, 450, "motm 111, mtf 193", New List(Of String) From {"", ""}),
        New Creature("Ettercap", "Medium", "Monstrosity", "NE", 2, 450, "mm 131", New List(Of String) From {"Forest", ""}),
        New Creature("Gargoyle", "Medium", "Elemental", "CE", 2, 450, "mm 140", New List(Of String) From {"", ""}),
        New Creature("Gelatinous Cube", "Large", "Ooze", "Unaligned", 2, 450, "mm 242", New List(Of String) From {"", ""}),
        New Creature("Ghast", "Medium", "Undead", "CE", 2, 450, "mm 148", New List(Of String) From {"", ""}),
        New Creature("Giant Boar", "Large", "Beast", "Unaligned", 2, 450, "mm 323", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Giant Constrictor Snake", "Huge", "Beast", "Unaligned", 2, 450, "mm 324", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Giant Elk", "Huge", "Beast", "Unaligned", 2, 450, "mm 325", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Gibbering Mouther", "Medium", "Aberration", "N", 2, 450, "mm 157", New List(Of String) From {"", ""}),
        New Creature("Githzerai Monk", "Medium", "Humanoid (gith)", "LN", 2, 450, "mm 161", New List(Of String) From {"", ""}),
        New Creature("Gnoll Pack Lord", "Medium", "Humanoid (gnoll)", "CE", 2, 450, "mm 163", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Green Dragon Wyrmling", "Medium", "Dragon", "LE", 2, 450, "mm 95", New List(Of String) From {"", ""}),
        New Creature("Grick", "Medium", "Monstrosity", "N", 2, 450, "mm 173", New List(Of String) From {"Forest", ""}),
        New Creature("Griffon", "Large", "Monstrosity", "Unaligned", 2, 450, "mm 174", New List(Of String) From {"Arctic", "Coastal", "Grassland"}),
        New Creature("Grung Elite Warrior", "Small", "Humanoid (grung)", "LE", 2, 450, "motm 150, vgm 157", New List(Of String) From {"Forest", ""}),
        New Creature("Guard Drake", "Medium", "Dragon", "Unaligned", 2, 450, "motm 151, vgm 158", New List(Of String) From {"Arctic", "Desert", "Forest"}),
        New Creature("Hobgoblin Iron Shadow", "Medium", "Humanoid (goblinoid)", "LE", 2, 450, "motm 154, vgm 162", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Hunter Shark", "Large", "Beast", "Unaligned", 2, 450, "mm 330", New List(Of String) From {"", ""}),
        New Creature("Intellect Devourer", "Tiny", "Aberration", "LE", 2, 450, "mm 191", New List(Of String) From {"", ""}),
        New Creature("Lizardfolk Shaman", "Medium", "Humanoid (lizardfolk)", "N", 2, 450, "mm 205", New List(Of String) From {"Forest", ""}),
        New Creature("Meenlock", "Small", "Fey", "NE", 2, 450, "motm 178, vgm 170", New List(Of String) From {"Forest", ""}),
        New Creature("Merrow", "Large", "Monstrosity", "CE", 2, 450, "mm 219", New List(Of String) From {"Coastal", ""}),
        New Creature("Mimic", "Medium", "Monstrosity (shapechanger)", "N", 2, 450, "mm 220", New List(Of String) From {"", ""}),
        New Creature("Minotaur Skeleton", "Large", "Undead", "LE", 2, 450, "mm 273", New List(Of String) From {"", ""}),
        New Creature("Myconid Sovereign", "Large", "Plant", "LN", 2, 450, "mm 232", New List(Of String) From {"", ""}),
        New Creature("Nothic", "Medium", "Aberration", "NE", 2, 450, "mm 236", New List(Of String) From {"", ""}),
        New Creature("Ochre Jelly", "Large", "Ooze", "Unaligned", 2, 450, "mm 243", New List(Of String) From {"", ""}),
        New Creature("Ogre", "Large", "Giant", "CE", 2, 450, "mm 237", New List(Of String) From {"Arctic", "Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Ogre Bolt Launcher", "Large", "Giant", "CE", 2, 450, "motm 200, mtf 220", New List(Of String) From {"Grassland", ""}),
        New Creature("Ogre Howdah", "Large", "Giant", "CE", 2, 450, "motm 201, mtf 221", New List(Of String) From {"Grassland", ""}),
        New Creature("Ogre Zombie", "Large", "Undead", "NE", 2, 450, "mm 316", New List(Of String) From {"", ""}),
        New Creature("Orc Claw of Luthic", "Medium", "Humanoid (orc)", "CE", 2, 450, "vgm 183", New List(Of String) From {"", ""}),
        New Creature("Orc Eye of Gruumsh", "Medium", "Humanoid (orc)", "CE", 2, 450, "mm 247", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Orc Hand of Yurtrus", "Medium", "Humanoid (orc)", "CE", 2, 450, "vgm 184", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Orog", "Medium", "Humanoid (orc)", "CE", 2, 450, "mm 247", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Pegasus", "Large", "Celestial", "CG", 2, 450, "mm 250", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Pentadrone", "Large", "Construct", "LN", 2, 450, "mm 226", New List(Of String) From {"", ""}),
        New Creature("Peryton", "Medium", "Monstrosity", "CE", 2, 450, "mm 251", New List(Of String) From {"", ""}),
        New Creature("Plesiosaurus", "Large", "Beast", "Unaligned", 2, 450, "mm 80", New List(Of String) From {"Coastal", ""}),
        New Creature("Polar Bear", "Large", "Beast", "Unaligned", 2, 450, "mm 334", New List(Of String) From {"Arctic", ""}),
        New Creature("Priest", "Medium", "Humanoid", "Any", 2, 450, "mm 348", New List(Of String) From {"", ""}),
        New Creature("Quaggoth", "Medium", "Humanoid (quaggoth)", "CN", 2, 450, "mm 256", New List(Of String) From {"", ""}),
        New Creature("Quetzalcoatlus", "Huge", "Beast", "Unaligned", 2, 450, "motm 96, vgm 140", New List(Of String) From {"Coastal", ""}),
        New Creature("Rhinoceros", "Large", "Beast", "Unaligned", 2, 450, "mm 336", New List(Of String) From {"Grassland", ""}),
        New Creature("Rug of Smothering", "Large", "Construct", "Unaligned", 2, 450, "mm 20", New List(Of String) From {"", ""}),
        New Creature("Rutterkin", "Medium", "Fiend (demon)", "CE", 2, 450, "motm 210, mtf 136", New List(Of String) From {"", ""}),
        New Creature("Saber-Toothed Tiger", "Large", "Beast", "Unaligned", 2, 450, "mm 336", New List(Of String) From {"Arctic", ""}),
        New Creature("Sahuagin Priestess", "Medium", "Humanoid (sahuagin)", "LE", 2, 450, "mm 264", New List(Of String) From {"Coastal", ""}),
        New Creature("Sea Hag", "Medium", "Fey", "CE", 2, 450, "mm 179", New List(Of String) From {"Coastal", ""}),
        New Creature("Shadow Mastiff", "Medium", "Monstrosity", "NE", 2, 450, "motm 215, vgm 190", New List(Of String) From {"Forest", ""}),
        New Creature("Silver Dragon Wyrmling", "Medium", "Dragon", "LG", 2, 450, "mm 118", New List(Of String) From {"", ""}),
        New Creature("Specter", "Medium", "Undead (poltergeist)", "CE", 2, 450, "mm 279", New List(Of String) From {"", ""}),
        New Creature("Spined Devil", "Small", "Fiend (devil)", "LE", 2, 450, "mm 78", New List(Of String) From {"", ""}),
        New Creature("Swarm of Poisonous Snakes", "Medium", "Beast", "Unaligned", 2, 450, "mm 338", New List(Of String) From {"Forest", ""}),
        New Creature("Tortle Druid", "Medium", "Humanoid (tortle)", "LN", 2, 450, "motm 244, mtf 242", New List(Of String) From {"Coastal", ""}),
        New Creature("Vegepygmy Chief", "Small", "Plant", "N", 2, 450, "motm 253, vgm 197", New List(Of String) From {"Forest", ""}),
        New Creature("Wererat", "Medium", "Humanoid (human,shapechanger)", "LE", 2, 450, "mm 209", New List(Of String) From {"Forest", ""}),
        New Creature("White Dragon Wyrmling", "Medium", "Dragon", "CE", 2, 450, "mm 102", New List(Of String) From {"", ""}),
        New Creature("Will-O'-Wisp", "Tiny", "Undead", "CE", 2, 450, "mm 301", New List(Of String) From {"Forest", ""}),
        New Creature("Yuan-Ti Broodguard", "Medium", "Humanoid (yuan-ti)", "NE", 2, 450, "motm 273, vgm 203", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Ankylosaurus", "Huge", "Beast", "Unaligned", 3, 700, "mm 79", New List(Of String) From {"Grassland", ""}),
        New Creature("Archer", "Medium", "Humanoid", "Any", 2, 450, "motm 49, vgm 210", New List(Of String) From {"", ""}),
        New Creature("Basilisk", "Medium", "Monstrosity", "Unaligned", 3, 700, "mm 24", New List(Of String) From {"", ""}),
        New Creature("Bearded Devil", "Medium", "Fiend (devil)", "LE", 3, 700, "mm 70", New List(Of String) From {"", ""}),
        New Creature("Blue Dragon Wyrmling", "Medium", "Dragon", "LE", 3, 700, "mm 91", New List(Of String) From {"", ""}),
        New Creature("Bugbear Chief", "Medium", "Humanoid (goblinoid)", "CE", 3, 700, "mm 33", New List(Of String) From {"", ""}),
        New Creature("Bulezau", "Medium", "Fiend (demon)", "CE", 3, 700, "motm 67, mtf 131", New List(Of String) From {"", ""}),
        New Creature("Cave Fisher", "Medium", "Monstrosity", "Unaligned", 3, 700, "motm 73, vgm 130", New List(Of String) From {"", ""}),
        New Creature("Choldrith", "Medium", "Monstrosity", "CE", 3, 700, "motm 77, vgm 132", New List(Of String) From {"", ""}),
        New Creature("Deathlock Wight", "Medium", "Undead", "NE", 3, 700, "motm 87, mtf 129", New List(Of String) From {"", ""}),
        New Creature("Deep Scion", "Medium", "Humanoid (shapechanger)", "NE", 3, 700, "motm 88, vgm 135", New List(Of String) From {"Coastal", ""}),
        New Creature("Derro Savant", "Small", "Humanoid (derro)", "CE", 3, 700, "motm 92, mtf 159", New List(Of String) From {"", ""}),
        New Creature("Displacer Beast", "Large", "Monstrosity", "LE", 3, 700, "mm 81", New List(Of String) From {"Forest", ""}),
        New Creature("Dolphin Delighter", "Medium", "Fey", "CG", 3, 700, "motm 97", New List(Of String) From {"Coastal", ""}),
        New Creature("Doppelganger", "Medium", "Monstrosity (shapechanger)", "N", 3, 700, "mm 82", New List(Of String) From {"", ""}),
        New Creature("Duergar Screamer", "Medium", "Construct", "LE", 3, 700, "motm 112, mtf 190", New List(Of String) From {"", ""}),
        New Creature("Flail Snail", "Large", "Elemental", "Unaligned", 3, 700, "motm 126, vgm 144", New List(Of String) From {"Forest", ""}),
        New Creature("Giant Scorpion", "Large", "Beast", "Unaligned", 3, 700, "mm 327", New List(Of String) From {"Desert", ""}),
        New Creature("Giff", "Medium", "Humanoid", "LN", 3, 700, "motm 138, mtf 204", New List(Of String) From {"", ""}),
        New Creature("Githyanki Warrior", "Medium", "Humanoid (gith)", "LE", 3, 700, "mm 160", New List(Of String) From {"", ""}),
        New Creature("Gold Dragon Wyrmling", "Medium", "Dragon", "LG", 3, 700, "mm 115", New List(Of String) From {"", ""}),
        New Creature("Green Hag", "Medium", "Fey", "NE", 3, 700, "mm 177", New List(Of String) From {"Forest", ""}),
        New Creature("Grell", "Medium", "Aberration", "NE", 3, 700, "mm 172", New List(Of String) From {"", ""}),
        New Creature("Hell Hound", "Medium", "Fiend", "LE", 3, 700, "mm 182", New List(Of String) From {"", ""}),
        New Creature("Hobgoblin Captain", "Medium", "Humanoid (goblinoid)", "LE", 3, 700, "mm 186", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Hook Horror", "Large", "Monstrosity", "N", 3, 700, "mm 189", New List(Of String) From {"", ""}),
        New Creature("Illusionist", "Medium", "Humanoid", "Any", 3, 700, "motm 263, vgm 214", New List(Of String) From {"", ""}),
        New Creature("Killer Whale", "Huge", "Beast", "Unaligned", 3, 700, "mm 331", New List(Of String) From {"", ""}),
        New Creature("Knight", "Medium", "Humanoid", "Any", 3, 700, "mm 347", New List(Of String) From {"", ""}),
        New Creature("Leucrotta", "Large", "Monstrosity", "CE", 3, 700, "motm 170, vgm 169", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Manticore", "Large", "Monstrosity", "LE", 3, 700, "mm 213", New List(Of String) From {"Arctic", "Coastal", "Grassland"}),
        New Creature("Martial Arts Adept", "Medium", "Humanoid", "Any", 3, 700, "motm 172, vgm 216", New List(Of String) From {"", ""}),
        New Creature("Merrenoloth", "Medium", "Fiend (yugoloth)", "NE", 3, 700, "motm 180, mtf 250", New List(Of String) From {"Coastal", ""}),
        New Creature("Minotaur", "Large", "Monstrosity", "CE", 3, 700, "mm 223", New List(Of String) From {"", ""}),
        New Creature("Mummy", "Medium", "Undead", "LE", 3, 700, "mm 228", New List(Of String) From {"Desert", ""}),
        New Creature("Neogi", "Small", "Aberration", "LE", 3, 700, "motm 192, vgm 180", New List(Of String) From {"", ""}),
        New Creature("Nightmare", "Large", "Fiend", "NE", 3, 700, "mm 235", New List(Of String) From {"", ""}),
        New Creature("Ogre Chain Brute", "Large", "Giant", "CE", 3, 700, "motm 201, mtf 221", New List(Of String) From {"Grassland", ""}),
        New Creature("Orc Red Fang of Shargaas", "Medium", "Humanoid (orc)", "CE", 3, 700, "vgm 185", New List(Of String) From {"Forest", ""}),
        New Creature("Owlbear", "Large", "Monstrosity", "Unaligned", 3, 700, "mm 249", New List(Of String) From {"Forest", ""}),
        New Creature("Phase Spider", "Large", "Monstrosity", "Unaligned", 3, 700, "mm 334", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Quaggoth Thonot", "Medium", "Humanoid (quaggoth)", "CN", 3, 700, "mm 256", New List(Of String) From {"", ""}),
        New Creature("Redcap", "Small", "Fey", "CE", 3, 700, "motm 208, vgm 188", New List(Of String) From {"Forest", ""}),
        New Creature("Shadow Mastiff Alpha", "Medium", "Monstrosity", "NE", 3, 700, "motm 215", New List(Of String) From {"Forest", ""}),
        New Creature("Slithering Tracker", "Medium", "Ooze", "CE", 3, 700, "motm 221, vgm 191", New List(Of String) From {"", ""}),
        New Creature("Spectator", "Medium", "Aberration", "LN", 3, 700, "mm 30", New List(Of String) From {"", ""}),
        New Creature("Swashbuckler", "Medium", "Humanoid", "Any non-lawful", 3, 700, "motm 238, vgm 217", New List(Of String) From {"", ""}),
        New Creature("Sword Wraith Warrior", "Medium", "Undead", "LE", 3, 700, "mot 239, mtf 241", New List(Of String) From {"Grassland", ""}),
        New Creature("Trapper", "Large", "Monstrosity", "Unaligned", 3, 700, "motm 245, vgm 194", New List(Of String) From {"", ""}),
        New Creature("Vampiric Mist", "Medium", "Undead", "CE", 3, 700, "motm 250, mtf 246", New List(Of String) From {"Arctic", "Coastal", "Forest", "Grassland"}),
        New Creature("Veteran", "Medium", "Humanoid", "Any", 3, 700, "mm 350", New List(Of String) From {"Arctic", "Coastal", "Forest", "Grassland"}),
        New Creature("Water Weird", "Large", "Elemental", "N", 3, 700, "mm 299", New List(Of String) From {"", ""}),
        New Creature("Werewolf", "Medium", "Humanoid (human,shapechanger)", "CE", 3, 700, "mm 211", New List(Of String) From {"Forest", ""}),
        New Creature("Wight", "Medium", "Undead", "NE", 3, 700, "mm 300", New List(Of String) From {"Desert", ""}),
        New Creature("Winter Wolf", "Large", "Monstrosity", "NE", 3, 700, "mm 340", New List(Of String) From {"Arctic", ""}),
        New Creature("Yeti", "Large", "Monstrosity", "CE", 3, 700, "mm 305", New List(Of String) From {"Arctic", ""}),
        New Creature("Yuan-Ti Malison", "Medium", "Monstrosity (shapechanger,yuan-ti)", "NE", 3, 700, "mm 309", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Babau", "Medium", "Fiend (demon)", "CE", 4, 1100, "motm 52, vgm 136", New List(Of String) From {"", ""}),
        New Creature("Banshee", "Medium", "Undead", "CE", 4, 1100, "mm 23", New List(Of String) From {"Coastal", "Forest"}),
        New Creature("Barghest", "Large", "Fiend (shapechanger)", "NE", 4, 1100, "motm 60, vgm 123", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Black Pudding", "Large", "Ooze", "Unaligned", 4, 1100, "mm 241", New List(Of String) From {"", ""}),
        New Creature("Bone Naga", "Large", "Undead", "LE", 4, 1100, "mm 233", New List(Of String) From {"", ""}),
        New Creature("Chuul", "Large", "Aberration", "CE", 4, 1100, "mm 40", New List(Of String) From {"", ""}),
        New Creature("Couatl", "Medium", "Celestial", "LG", 4, 1100, "mm 43", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Deathlock", "Medium", "Undead", "NE", 4, 1100, "motm 86, mtf 128", New List(Of String) From {"", ""}),
        New Creature("Dybbuk", "Medium", "Fiend (demon)", "CE", 4, 1100, "motm 113, mtf 132", New List(Of String) From {"Desert", ""}),
        New Creature("Elephant", "HUge", "Beast", "Unaligned", 4, 1100, "mm 322", New List(Of String) From {"Grassland", ""}),
        New Creature("Ettin", "Large", "Giant", "CE", 4, 1100, "mm 132", New List(Of String) From {"", ""}),
        New Creature("Flameskull", "Tiny", "Undead", "NE", 4, 1100, "mm 134", New List(Of String) From {"", ""}),
        New Creature("Ghost", "Medium", "Undead", "Any", 4, 1100, "mm 147", New List(Of String) From {"", ""}),
        New Creature("Girallon", "Large", "Monstrosity", "Unaligned", 4, 1100, "motm 139, vgm 152", New List(Of String) From {"Forest", ""}),
        New Creature("Gnoll Fang of Yeenoghu", "Medium", "Fiend (gnoll)", "CE", 4, 1100, "mm 163", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Helmed Horror", "Medium", "Construct", "N", 4, 1100, "mm 183", New List(Of String) From {"", ""}),
        New Creature("Hobgoblin Devastator", "Medium", "Humanoid (goblinoid)", "LE", 4, 1100, "motm 153, vgm 161", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Incubus", "Medium", "Fiend (shapechanger)", "NE", 4, 1100, "mm 285", New List(Of String) From {"", ""}),
        New Creature("Iron Cobra", "Medium", "Construct", "Unaligned", 4, 1100, "motm 79, mtf 125", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Lamia", "Large", "Monstrosity", "CE", 4, 1100, "mm 201", New List(Of String) From {"Desert", ""}),
        New Creature("Lizard King/Queen", "Medium", "Humanoid (lizardfolk)", "CE", 4, 1100, "mm 205", New List(Of String) From {"", ""}),
        New Creature("Merregon", "Medium", "Fiend (devil)", "LE", 4, 1100, "motm 179, mtf 166", New List(Of String) From {"", ""}),
        New Creature("Neogi Master", "Medium", "Aberration", "LE", 4, 1100, "motm 192, vgm 180", New List(Of String) From {"", ""}),
        New Creature("Ogre Battering Ram", "Large", "Giant", "CE", 4, 1100, "motm 200, mtf 220", New List(Of String) From {"Grassland", ""}),
        New Creature("Orc Blade of Ilneval", "Medium", "Humanoid (orc)", "CE", 4, 1100, "vgm 183", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Orc War Chief", "Medium", "Humanoid (orc)", "CE", 4, 1100, "mm 246", New List(Of String) From {"", ""}),
        New Creature("Red Dragon Wyrmling", "Medium", "Dragon", "CE", 4, 1100, "mm 98", New List(Of String) From {"", ""}),
        New Creature("Shadow Demon", "Medium", "Fiend (demon)", "CE", 4, 1100, "mm 64", New List(Of String) From {"", ""}),
        New Creature("Stegosaurus", "Huge", "Beast", "Unaligned", 4, 1100, "motm 96, vgm 140", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Stone Defender", "Medium", "Construct", "Unaligned", 4, 1100, "motm 80, mtf 126", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Succubus", "Medium", "Fiend (shapechanger)", "NE", 4, 1100, "mm 285", New List(Of String) From {"", ""}),
        New Creature("Warlock of the Archfey", "Medium", "Humanoid", "Any", 4, 1100, "motm 255, vgm 219", New List(Of String) From {"", ""}),
        New Creature("Wereboar", "Medium", "Humanoid (human,shapechanger)", "NE", 4, 1100, "mm 209", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Weretiger", "Medium", "Humanoid (human,shapechanger)", "N", 4, 1100, "mm 210", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Yeth Hound", "Large", "Fey", "NE", 4, 1100, "motm 271, vgm 201", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Yuan-Ti Mind Whisperer", "Medium", "Monstrosity (shapechanger,yuan-ti)", "NE", 4, 1100, "motm 274, vgm 204", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Yuan-Ti Nightmare Speaker", "Medium", "Monstrosity (shapechanger,yuan-ti)", "NE", 4, 1100, "motm 275, vgm 205", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Adult Oblex", "Medium", "Ooze", "LE", 5, 1800, "motm 198, mtf 218", New List(Of String) From {"", ""}),
        New Creature("Air Elemental", "Large", "Elmental", "N", 5, 1800, "mm 124", New List(Of String) From {"Desert", ""}),
        New Creature("Allip", "Medium", "Undead", "NE", 5, 1800, "motm 45, mtf 116", New List(Of String) From {"", ""}),
        New Creature("Banderhobb", "Large", "Monstrosity", "NE", 5, 1800, "motm 56, vgm 122", New List(Of String) From {"", ""}),
        New Creature("Barbed Devil", "Medium", "Fiend (devil)", "LE", 5, 1800, "mm 70", New List(Of String) From {"", ""}),
        New Creature("Barlgura", "Large", "Fiend (devil)", "CE", 5, 1800, "mm 56", New List(Of String) From {"", ""}),
        New Creature("Beholder Zombie", "Large", "Undead", "NE", 5, 1800, "mm 316", New List(Of String) From {"", ""}),
        New Creature("Brontosaurus", "Gargantuan", "Beast", "Unaligned", 5, 1800, "motm 95, vgm 139", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Bulette", "Large", "Monstrosity", "Unaligned", 5, 1800, "mm 34", New List(Of String) From {"Grassland", ""}),
        New Creature("Cambion", "Medium", "Fiend", "Any evil", 5, 1800, "mm 36", New List(Of String) From {"", ""}),
        New Creature("Catoblepas", "Large", "Monstrosity", "Unaligned", 5, 1800, "motm 70, vgm 129", New List(Of String) From {"", ""}),
        New Creature("Drow Elite Warrior", "Medium", "Humanoid (elf)", "NE", 5, 1800, "mm 128", New List(Of String) From {"", ""}),
        New Creature("Earth Elemental", "Large", "Elemental", "N", 5, 1800, "mm 124", New List(Of String) From {"", ""}),
        New Creature("Enchanter", "Medium", "Humanoid", "Any", 5, 1800, "motm 261, vgm 213", New List(Of String) From {"", ""}),
        New Creature("Fire Elemental", "Large", "Elemental", "N", 5, 1800, "mm 125", New List(Of String) From {"Desert", ""}),
        New Creature("Flesh Golem", "Medium", "Construct", "N", 5, 1800, "mm 169", New List(Of String) From {"", ""}),
        New Creature("Giant Crocodile", "Huge", "Beast", "Unaligned", 5, 1800, "mm 324", New List(Of String) From {"", ""}),
        New Creature("Giant Shark", "Huge", "Beast", "Unaligned", 5, 1800, "mm 328", New List(Of String) From {"", ""}),
        New Creature("Gladiator", "Medium", "Humanoid", "Any", 5, 1800, "mm 346", New List(Of String) From {"", ""}),
        New Creature("Gorgon", "Large", "Monstrosity", "Unaligned", 5, 1800, "mm 171", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Hill Giant", "Huge", "Giant", "CE", 5, 1800, "mm 155", New List(Of String) From {"", ""}),
        New Creature("Kraken Priest", "Medium", "Humanoid", "Any", 5, 1800, "motm 167, vgm 215", New List(Of String) From {"", ""}),
        New Creature("Kruthik Hive Lord", "Large", "Monstrosity", "Unaligned", 5, 1800, "motm 169, mtf 212", New List(Of String) From {"Desert", ""}),
        New Creature("Master Thief", "Medium", "Humanoid", "Any", 5, 1800, "motm 174, vgm 216", New List(Of String) From {"", ""}),
        New Creature("Mezzoloth", "Medium", "Fiend (yugoloth)", "NE", 5, 1800, "mm 313", New List(Of String) From {"", ""}),
        New Creature("Mindwitness", "Large", "Aberration", "LE", 5, 1800, "motm 181, vgm 176", New List(Of String) From {"", ""}),
        New Creature("Night Hag", "Medium", "Fiend", "NE", 5, 1800, "mm 178", New List(Of String) From {"", ""}),
        New Creature("Oaken Bolter", "Medium", "Construct", "Unaligned", 5, 1800, "motm 80, mtf 126", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Otyugh", "Large", "Aberration", "N", 5, 1800, "mm 248", New List(Of String) From {"", ""}),
        New Creature("Red Slaad", "Large", "Aberration", "CN", 5, 1800, "mm 276", New List(Of String) From {"", ""}),
        New Creature("Revenant", "Medium", "Undead", "N", 5, 1800, "mm 259", New List(Of String) From {"Arctic", "Desert", "Forest"}),
        New Creature("Roper", "Large", "Monstrosity", "NE", 5, 1800, "mm 261", New List(Of String) From {"", ""}),
        New Creature("Sahuagin Baron", "Large", "Humanoid (sahuagin)", "LE", 5, 1800, "mm 264", New List(Of String) From {"Coastal", ""}),
        New Creature("Salamander", "Large", "Elemental", "NE", 5, 1800, "mm 266", New List(Of String) From {"", ""}),
        New Creature("Shambling Mound", "Large", "Plant", "Unaligned", 5, 1800, "mm 270", New List(Of String) From {"Forest", ""}),
        New Creature("Spawn of Kyuss", "Medium", "Undead", "CE", 5, 1800, "motm 225, vgm 192", New List(Of String) From {"Desert", ""}),
        New Creature("Star Spawn Mangler", "Medium", "Aberration", "CE", 5, 1800, "motm 229, mtf 236", New List(Of String) From {"", ""}),
        New Creature("Swarm of Cranium Rats", "Medium", "Beast", "LE", 5, 1800, "motm 83, vgm 133", New List(Of String) From {"", ""}),
        New Creature("Tanarukk", "Medium", "Fiend (demon,orc)", "CE", 5, 1800, "motm 240, vgm 186", New List(Of String) From {"", ""}),
        New Creature("Tlincalli", "Large", "Monstrosity", "NE", 5, 1800, "motm 243, vgm 193", New List(Of String) From {"Desert", ""}),
        New Creature("Transmuter", "Medium", "Humanoid", "Any", 5, 1800, "motm 265, vgm 218", New List(Of String) From {"", ""}),
        New Creature("Triceratops", "Huge", "Beast", "Unaligned", 5, 1800, "mm 80", New List(Of String) From {"Grassland", ""}),
        New Creature("Troll", "Large", "Giant", "CE", 5, 1800, "mm 291", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Umber Hulk", "Large", "Monstrosity", "CE", 5, 1800, "mm 292", New List(Of String) From {"", ""}),
        New Creature("Unicorn", "Large", "Celestial", "LG", 5, 1800, "mm 294", New List(Of String) From {"Forest", ""}),
        New Creature("Vampire Spawn", "Medium", "Undead", "NE", 5, 1800, "mm 298", New List(Of String) From {"", ""}),
        New Creature("Water Elemental", "Large", "Elemental", "N", 5, 1800, "mm 125", New List(Of String) From {"Coastal", ""}),
        New Creature("Werebear", "Medium", "Humanoid (human,shapechanger)", "NG", 5, 1800, "mm 208", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Wood Woad", "Medium", "Plant", "LN", 5, 1800, "motm 266, vgm 198", New List(Of String) From {"Forest", ""}),
        New Creature("Wraith", "Medium", "Undead", "NE", 5, 1800, "mm 302", New List(Of String) From {"", ""}),
        New Creature("Xorn", "Medium", "Elemental", "N", 5, 1800, "mm 304", New List(Of String) From {"", ""}),
        New Creature("Young Remorhaz", "Large", "Monstrosity", "Unaligned", 5, 1800, "mm 258", New List(Of String) From {"Arctic", ""}),
        New Creature("Yuan-Ti Pit Master", "Medium", "Monstrosity (shapechanger,yuan-ti)", "NE", 5, 1800, "motm 276, vgm 206", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Annis Hag", "Large", "Fey", "CE", 6, 2300, "motm 47, vgm 159", New List(Of String) From {"", ""}),
        New Creature("Bodak", "Medium", "Undead", "CE", 6, 2300, "motm 64, vgm 127", New List(Of String) From {"", ""}),
        New Creature("Chasme", "Large", "Fiend (demon)", "CE", 6, 2300, "mm 57", New List(Of String) From {"", ""}),
        New Creature("Chimera", "Large", "Monstrosity", "CE", 6, 2300, "mm 39", New List(Of String) From {"Grassland", ""}),
        New Creature("Conjurer", "Medium", "Humanoid", "Any", 6, 2300, "motm 260, vgm 212", New List(Of String) From {"", ""}),
        New Creature("Cyclops", "Huge", "Giant", "CN", 6, 2300, "mm 45", New List(Of String) From {"Coastal", "Desert", "Grassland"}),
        New Creature("Drider", "Large", "Monstrosity", "CE", 6, 2300, "mm 120", New List(Of String) From {"", ""}),
        New Creature("Duergar Warlord", "Medium", "Humanoid (dwarf)", "LE", 6, 2300, "motm 111, mtf 192", New List(Of String) From {"", ""}),
        New Creature("Galeb Duhr", "Medium", "Elemental", "N", 6, 2300, "mm 139", New List(Of String) From {"", ""}),
        New Creature("Githzerai Zerth", "Medium", "Humanoid (gith)", "LN", 6, 2300, "mm 161", New List(Of String) From {"", ""}),
        New Creature("Gauth", "Medium", "Aberration", "LE", 6, 2300, "motm 133, vgm 125", New List(Of String) From {"", ""}),
        New Creature("Hobgoblin Warlord", "Medium", "Humanoid (goblinoid)", "LE", 6, 2300, "mm 187", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Invisible Stalker", "Medium", "Elemental", "N", 6, 2300, "mm 192", New List(Of String) From {"", ""}),
        New Creature("Kuo-Toa Archpriest", "Medium", "Humanoid (kuo-toa)", "NE", 6, 2300, "mm 200", New List(Of String) From {"", ""}),
        New Creature("Mage", "Medium", "Humanoid", "Any", 6, 2300, "mm 347", New List(Of String) From {"", ""}),
        New Creature("Mammoth", "Huge", "Beast", "Unaligned", 6, 2300, "mm 332", New List(Of String) From {"Arctic", ""}),
        New Creature("Medusa", "Medium", "Monstrosity", "LE", 6, 2300, "mm 214", New List(Of String) From {"Desert", ""}),
        New Creature("Mouth of Grolantor", "Huge", "Giant (hill giant)", "CE", 6, 2300, "motm 187, vgm 149", New List(Of String) From {"Grassland", ""}),
        New Creature("Vrock", "Large", "Fiend (demon)", "CE", 6, 2300, "mm 64", New List(Of String) From {"", ""}),
        New Creature("Warlock of the Great Old One", "Medium", "Humanoid", "Any", 6, 2300, "motm 256, vgm 220", New List(Of String) From {"", ""}),
        New Creature("White Abishai", "Medium", "Fiend (devil)", "LE", 6, 2300, "motm 41, mtf 163", New List(Of String) From {"", ""}),
        New Creature("Wyvern", "Large", "Dragon", "Unaligned", 6, 2300, "mm 303", New List(Of String) From {"", ""}),
        New Creature("Young Brass Dragon", "Large", "Dragon", "CG", 6, 2300, "mm 105", New List(Of String) From {"Desert", ""}),
        New Creature("Young White Dragon", "Large", "Dragon", "CE", 6, 2300, "mm 101", New List(Of String) From {"Arctic", ""}),
        New Creature("Air Elemental Myrmidon", "Medium", "Elemental", "N", 7, 2900, "motm 122, mtf 202", New List(Of String) From {"", ""}),
        New Creature("Armanite", "Large", "Fiend (demon)", "CE", 7, 2900, "motm 50, mtf 131", New List(Of String) From {"", ""}),
        New Creature("Bheur Hag", "Medium", "Fey", "CE", 7, 2900, "motm 62, vgm 160", New List(Of String) From {"Arctic", ""}),
        New Creature("Black Abishai", "Medium", "Fiend (devil)", "LE", 7, 2900, "motm 38, mtf 160", New List(Of String) From {"", ""}),
        New Creature("Blue Slaad", "Large", "Aberration", "CN", 7, 2900, "mm 276", New List(Of String) From {"", ""}),
        New Creature("Dhergoloth", "Medium", "Fiend (yugoloth)", "NE", 7, 2900, "motm 94, mtf 248", New List(Of String) From {"", ""}),
        New Creature("Draegloth", "Large", "Fiend (demon)", "CE", 7, 2900, "motm 98, vgm 141", New List(Of String) From {"", ""}),
        New Creature("Drow Mage", "Medium", "Humanoid (elf)", "NE", 7, 2900, "mm 129", New List(Of String) From {"", ""}),
        New Creature("Earth Elemental Myrmidon", "Medium", "Elemental", "N", 7, 2900, "motm 122, mtf 202", New List(Of String) From {"", ""}),
        New Creature("Fire Elemental Myrmidon", "Medium", "Elemental", "N", 7, 2900, "motm 123, mtf 203", New List(Of String) From {"", ""}),
        New Creature("Giant Ape", "Huge", "Beast", "Unaligned", 7, 2900, "mm 323", New List(Of String) From {"Forest", ""}),
        New Creature("Grick Alpha", "Large", "Monstrosity", "N", 7, 2900, "mm 173", New List(Of String) From {"Forest", ""}),
        New Creature("Korred", "Small", "Fey", "CN", 7, 2900, "motm 166, vgm 168", New List(Of String) From {"Forest", ""}),
        New Creature("Lost Sorrowsworn", "Medium", "Monstrosity", "NE", 7, 2900, "motm 224, mtf 233", New List(Of String) From {"Arctic", "Desert", "Forest"}),
        New Creature("Maurezhi", "Medium", "Fiend (demon)", "CE", 7, 2900, "motm 175, mtf 133", New List(Of String) From {"", ""}),
        New Creature("Mind Flayer", "Medium", "Aberration", "LE", 7, 2900, "mm 222", New List(Of String) From {"", ""}),
        New Creature("Oni", "Large", "Giant", "LE", 7, 2900, "mm 239", New List(Of String) From {"Forest", ""}),
        New Creature("Shield Guardian", "Large", "Construct", "Unaligned", 7, 2900, "mm 271", New List(Of String) From {"", ""}),
        New Creature("Stone Giant", "Huge", "Giant", "N", 7, 2900, "mm 156", New List(Of String) From {"", ""}),
        New Creature("Venom Troll", "Large", "Giant", "CE", 7, 2900, "motm 248, mtf 245", New List(Of String) From {"Forest", ""}),
        New Creature("Warlock of the Fiend", "Medium", "Humanoid", "Any", 7, 2900, "motm 255, vgm 219", New List(Of String) From {"", ""}),
        New Creature("Water Elemental Myrmidon", "Medium", "Elemental", "N", 7, 2900, "motm 123, mtf 203", New List(Of String) From {"", ""}),
        New Creature("Young Black Dragon", "Large", "Dragon", "CE", 7, 2900, "mm 88", New List(Of String) From {"", ""}),
        New Creature("Young Copper Dragon", "Large", "Dragon", "CG", 7, 2900, "mm 111", New List(Of String) From {"", ""}),
        New Creature("Yuan-Ti Abomination", "Large", "Monstrosity (shapechanger,yuan-ti)", "NE", 7, 2900, "mm 308", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Assassin", "Medium", "Humanoid", "Any non-good", 8, 3900, "mm 343", New List(Of String) From {"", ""}),
        New Creature("Blackguard", "Medium", "Humanoid", "Any non-good", 8, 3900, "motm 63, vgm 211", New List(Of String) From {"", ""}),
        New Creature("Canoloth", "Medium", "Fiend (yugoloth)", "NE", 8, 3900, "motm 69, mtf 247", New List(Of String) From {"Coastal", ""}),
        New Creature("Chain Devil", "Medium", "Fiend (devil)", "LE", 8, 3900, "mm 72", New List(Of String) From {"", ""}),
        New Creature("Cloaker", "Large", "Aberration", "CN", 8, 3900, "mm 41", New List(Of String) From {"", ""}),
        New Creature("Corpse Flower", "Large", "Plant", "CE", 8, 3900, "motm 82, mtf 127", New List(Of String) From {"Forest", ""}),
        New Creature("Deathlock Mastermind", "Medium", "Undead", "NE", 8, 3900, "motm 87, mtf 129", New List(Of String) From {"", ""}),
        New Creature("Diviner", "Medium", "Humanoid", "Any", 8, 3900, "motm 261, vgm 213", New List(Of String) From {"", ""}),
        New Creature("Drow Priestess of Lolth", "Medium", "Humanoid (elf)", "NE", 8, 3900, "mm 129", New List(Of String) From {"", ""}),
        New Creature("Fomorian", "Huge", "Giant", "CE", 8, 3900, "mm 136", New List(Of String) From {"", ""}),
        New Creature("Frost Giant", "Huge", "Giant", "NE", 8, 3900, "mm 155", New List(Of String) From {"Arctic", ""}),
        New Creature("Githyanki Knight", "Medium", "Humanoid (gith)", "LE", 8, 3900, "mm 160", New List(Of String) From {"", ""}),
        New Creature("Green Slaad", "Large", "Aberration (shapechanger)", "CN", 8, 3900, "mm 277", New List(Of String) From {"", ""}),
        New Creature("Hezrou", "Large", "Fiend (demon)", "CE", 8, 3900, "mm 60", New List(Of String) From {"", ""}),
        New Creature("Howler", "Large", "Fiend", "CE", 8, 3900, "motm 155, mtf 210", New List(Of String) From {"Desert", "Grassland"}),
        New Creature("Hydra", "Huge", "Monstrosity", "Unaligned", 8, 3900, "mm 190", New List(Of String) From {"", ""}),
        New Creature("Mind Flayer Arcanist", "Medium", "Abberation", "LE", 8, 3900, "mm 222", New List(Of String) From {"", ""}),
        New Creature("Shoosuva", "Large", "Fiend (demon)", "CE", 8, 3900, "motm 216, vgm 137", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Spirit Naga", "Large", "Monstrosity", "CE", 8, 3900, "mm 234", New List(Of String) From {"", ""}),
        New Creature("Sword Wraith Commander", "Medium", "Undead", "LE", 8, 3900, "motm 239, mtf 241", New List(Of String) From {"Grassland", ""}),
        New Creature("Tyrannosaurus Rex", "Huge", "Beast", "Unaligned", 8, 3900, "mm 80", New List(Of String) From {"Grassland", ""}),
        New Creature("Young Bronze Dragon", "Large", "Dragon", "LG", 8, 3900, "mm 108", New List(Of String) From {"Coastal", ""}),
        New Creature("Young Green Dragon", "Large", "Dragon", "LE", 8, 3900, "mm 94", New List(Of String) From {"Forest", ""}),
        New Creature("Abjurer", "Medium", "Humanoid", "Any", 9, 5000, "motm 260, vgm 209", New List(Of String) From {"", ""}),
        New Creature("Abominable Yeti", "Huge", "Monstrosity", "CE", 9, 5000, "mm 306", New List(Of String) From {"Arctic", ""}),
        New Creature("Bone Devil", "Large", "Fiend (devil)", "LE", 9, 5000, "mm 71", New List(Of String) From {"", ""}),
        New Creature("Champion", "Medium", "Humanoid", "Any", 9, 5000, "motm 74, vgm 212", New List(Of String) From {"", ""}),
        New Creature("Clay Golem", "Large", "Construct", "Unaligned", 9, 5000, "mm 168", New List(Of String) From {"", ""}),
        New Creature("Cloud Giant", "Huge", "Giant", "NG or NE", 9, 5000, "mm 154", New List(Of String) From {"", ""}),
        New Creature("Drow House Captain", "Medium", "Humanoid (elf)", "NE", 9, 5000, "motm 101, mtf 184", New List(Of String) From {"", ""}),
        New Creature("Evoker", "Medium", "Humanoid", "Any", 9, 5000, "motm 262, vgm 214", New List(Of String) From {"", ""}),
        New Creature("Fire Giant", "Huge", "Giant", "LE", 9, 5000, "mm 154", New List(Of String) From {"", ""}),
        New Creature("Flind", "Medium", "Humanoid (gnoll)", "CE", 9, 5000, "motm 127, vgm 153", New List(Of String) From {"Arctic", "Forest", "Grassland"}),
        New Creature("Frost Salamander", "Huge", "Elemental", "Unaligned", 9, 5000, "motm 132, mtf 223", New List(Of String) From {"Arctic", ""}),
        New Creature("Glabrezu", "Large", "Fiend (demon)", "CE", 9, 5000, "mm 58", New List(Of String) From {"", ""}),
        New Creature("Gray Slaad", "Medium", "Aberration (shapechanger)", "CN", 9, 5000, "mm 277", New List(Of String) From {"", ""}),
        New Creature("Hydroloth", "Medium", "Fiend (yugoloth)", "NE", 9, 5000, "motm 158, mtf 249", New List(Of String) From {"", ""}),
        New Creature("Lonely Sorrowsworn", "Medium", "Monstrosity", "NE", 9, 5000, "motm 223, mtf 232", New List(Of String) From {"Coastal", "Desert"}),
        New Creature("Necromancer", "Medium", "Humanoid", "Any", 9, 5000, "motm 264, vgm 217", New List(Of String) From {"", ""}),
        New Creature("Nycaloth", "Large", "Fiend (yugoloth)", "NE", 9, 5000, "mm 314", New List(Of String) From {"", ""}),
        New Creature("Rot Troll", "Large", "Giant", "CE", 9, 5000, "motm 247, mtf 244", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Treant", "Huge", "Plant", "CG", 9, 5000, "mm 289", New List(Of String) From {"Forest", ""}),
        New Creature("Ulitharid", "Large", "Aberration", "LE", 9, 5000, "motm 249, vgm 175", New List(Of String) From {"", ""}),
        New Creature("War Priest", "Medium", "Humanoid", "Any", 9, 5000, "motm 254, vgm 218", New List(Of String) From {"", ""}),
        New Creature("Young Blue Dragon", "Large", "Dragon", "LE", 9, 5000, "mm 91", New List(Of String) From {"Coastal", "Desert"}),
        New Creature("Young Silver Dragon", "Large", "Dragon", "LG", 9, 5000, "mm 118", New List(Of String) From {"", ""}),
        New Creature("Aboleth", "Large", "Aberration", "LE", 10, 5900, "mm 13", New List(Of String) From {"", ""}),
        New Creature("Alhoon", "Medium", "Undead", "Any evil", 10, 5900, "motm 43, vgm 172", New List(Of String) From {"", ""}),
        New Creature("Autumn Eladrin", "Medium", "Fey (elf)", "CN", 10, 5900, "motm 115, mtf 195", New List(Of String) From {"Forest", ""}),
        New Creature("Death Kiss", "Large", "Aberration", "NE", 10, 5900, "motm 85, vgm 124", New List(Of String) From {"", ""}),
        New Creature("Death Slaad", "Medium", "Aberration (shapechanger)", "CE", 10, 5900, "mm 278", New List(Of String) From {"", ""}),
        New Creature("Deva", "Medium", "Celestial", "LG", 10, 5900, "mm 16", New List(Of String) From {"", ""}),
        New Creature("Elder Oblex", "Huge", "Ooze", "LE", 10, 5900, "motm 199, mtf 219", New List(Of String) From {"", ""}),
        New Creature("Froghemoth", "Huge", "Monstrosity", "Unaligned", 10, 5900, "motm 130, vgm 145", New List(Of String) From {"", ""}),
        New Creature("Githyanki Gish", "Medium", "Humanoid (gith)", "LE", 10, 5900, "motm 140, mtf 205", New List(Of String) From {"Desert", ""}),
        New Creature("Githzerai Enlightened", "Medium", "Humanoid (gith)", "LN", 10, 5900, "motm 143, mtf 208", New List(Of String) From {"Desert", ""}),
        New Creature("Guardian Naga", "Large", "Monstrosity", "LG", 10, 5900, "mm 234", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Orthon", "Large", "Fiend (devil)", "LE", 10, 5900, "motm 205, mtf 169", New List(Of String) From {"Desert", ""}),
        New Creature("Spring Eladrin", "Medium", "Fey (elf)", "CN", 10, 5900, "motm 116, mtf 196", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Star Spawn Hulk", "Large", "Aberration", "CE", 10, 5900, "motm 227, mtf 234", New List(Of String) From {"", ""}),
        New Creature("Stone Giant Dreamwalker", "Huge", "Giant (stone giant)", "CN", 10, 5900, "motm 234, vgm 150", New List(Of String) From {"Coastal", ""}),
        New Creature("Stone Golem", "Large", "Construct", "Unaligned", 10, 5900, "mm 170", New List(Of String) From {"", ""}),
        New Creature("Summer Eladrin", "Medium", "Fey (elf)", "CN", 10, 5900, "motm 116, mtf 196", New List(Of String) From {"Forest", ""}),
        New Creature("Winter Eladrin", "Medium", "Fey (elf)", "CN", 10, 5900, "motm 117, mtf 197", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Yochlol", "Medium", "Fiend (demon,shapechanger)", "CE", 10, 5900, "mm 65", New List(Of String) From {"", ""}),
        New Creature("Young Gold Dragon", "Large", "Dragon", "LG", 10, 5900, "mm 115", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Young Red Dragon", "Large", "Dragon", "CE", 10, 5900, "mm 98", New List(Of String) From {"", ""}),
        New Creature("Alkilith", "Medium", "Fiend (demon)", "CE", 11, 7200, "motm 44, mtf 130", New List(Of String) From {"", ""}),
        New Creature("Balhannoth", "Large", "Aberration", "CE", 11, 7200, "motm 55, mtf 118", New List(Of String) From {"Coastal", ""}),
        New Creature("Behir", "Huge", "Monstrosity", "NE", 11, 7200, "mm 25", New List(Of String) From {"", ""}),
        New Creature("Cloud Giant Smiling One", "Huge", "Giant (cloud giant)", "CN", 11, 7200, "motm 81, vgm 146", New List(Of String) From {"", ""}),
        New Creature("Dao", "Large", "Elemental", "NE", 11, 7200, "mm 143", New List(Of String) From {"", ""}),
        New Creature("Djinni", "Large", "Elemental", "CG", 11, 7200, "mm 144", New List(Of String) From {"Coastal", ""}),
        New Creature("Drow Shadowblade", "Medium", "Humanoid (elf)", "NE", 11, 7200, "motm 105, mtf 187", New List(Of String) From {"", ""}),
        New Creature("Efreeti", "Large", "Elemental", "LE", 11, 7200, "mm 145", New List(Of String) From {"Desert", ""}),
        New Creature("Gynosphinx", "Large", "Monstrosity", "LN", 11, 7200, "mm 282", New List(Of String) From {"Desert", ""}),
        New Creature("Horned Devil", "Large", "Fiend (devil)", "LE", 11, 7200, "mm 74", New List(Of String) From {"", ""}),
        New Creature("Hungry Sorrowsworn", "Medium", "Monstrosity", "NE", 11, 7200, "motm 223, mtf 232", New List(Of String) From {"Forest", ""}),
        New Creature("Marid", "Large", "Elemental", "CN", 11, 7200, "mm 146", New List(Of String) From {"Coastal", ""}),
        New Creature("Morkoth", "Medium", "Aberration", "CE", 11, 7200, "motm 186, vgm 178", New List(Of String) From {"Coastal", ""}),
        New Creature("Remorhaz", "Huge", "Monstrosity", "Unaligned", 11, 7200, "mm 258", New List(Of String) From {"Arctic", ""}),
        New Creature("Roc", "Gargantuan", "Monstrosity", "Unaligned", 11, 7200, "mm 260", New List(Of String) From {"Arctic", "Coastal", "Desert"}),
        New Creature("Spirit Troll", "Large", "Giant", "CE", 11, 7200, "motm 247, mtf 244", New List(Of String) From {"Coastal", "Forest"}),
        New Creature("Yagnoloth", "Large", "Fiend (yugoloth)", "NE", 11, 7200, "motm 268, mtf 252", New List(Of String) From {"", ""}),
        New Creature("Arcanaloth", "Medium", "Fiend (yugoloth)", "NE", 12, 8400, "mm 313", New List(Of String) From {"", ""}),
        New Creature("Archdruid", "Medium", "Humanoid", "Any", 12, 8400, "motm 48, vgm 210", New List(Of String) From {"", ""}),
        New Creature("Archmage", "Medium", "Humanoid", "Any", 12, 8400, "mm 342", New List(Of String) From {"", ""}),
        New Creature("Boneclaw", "Large", "Undead", "CE", 12, 8400, "motm 66, mtf 121", New List(Of String) From {"Arctic", "Desert"}),
        New Creature("Duergar Despot", "Medium", "Humanoid (dwarf)", "LE", 12, 8400, "motm 107, mtf 188", New List(Of String) From {"", ""}),
        New Creature("Eidolon", "Medium", "Undead", "Any", 12, 8400, "motm 114, mtf 194", New List(Of String) From {"Coastal", "Desert", "Forest", "Grassland"}),
        New Creature("Erinyes", "Medium", "Fiend (devil)", "LE", 12, 8400, "mm 73", New List(Of String) From {"", ""}),
        New Creature("Frost Giant Everlasting One", "Huge", "Giant (frost giant)", "CE", 12, 8400, "motm 131, vgm 148", New List(Of String) From {"Arctic", "Coastal"}),
        New Creature("Githyanki Kith'Rak", "Medium", "Humanoid (gith)", "LE", 12, 8400, "motm 140, mtf 205", New List(Of String) From {"Desert", ""}),
        New Creature("Gray Render", "Large", "Monstrosity", "CN", 12, 8400, "motm 146, mtf 209", New List(Of String) From {"Forest", ""}),
        New Creature("Ki-Rin", "Huge", "Celestial", "LG", 12, 8400, "motm 162, vgm 163", New List(Of String) From {"Coastal", "Desert", "Grassland"}),
        New Creature("Oinoloth", "Medium", "Fiend (yugoloth)", "NE", 12, 8400, "motm 202, mtf 251", New List(Of String) From {"Desert", ""}),
        New Creature("Warlord", "Medium", "Humanoid", "Any", 12, 8400, "motm 257, vgm 220", New List(Of String) From {"", ""}),
        New Creature("Yuan-Ti Anathema", "Huge", "Monstrosity (shapechanger,yuan-ti)", "NE", 12, 8400, "motm 272, vgm 202", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Adult Brass Dragon", "Huge", "Dragon", "CG", 13, 10000, "mm 105", New List(Of String) From {"Desert", ""}),
        New Creature("Adult White Dragon", "Huge", "Dragon", "CE", 13, 10000, "mm 101", New List(Of String) From {"Arctic", ""}),
        New Creature("Angry Sorrowsworn", "Medium", "Monstrosity", "NE", 13, 10000, "motm 222, mtf 231", New List(Of String) From {"", ""}),
        New Creature("Beholder", "Large", "Aberration", "LE", 13, 10000, "mm 28", New List(Of String) From {"", ""}),
        New Creature("Devourer", "Large", "Fiend", "CE", 13, 10000, "motm 93, vgm 138", New List(Of String) From {"", ""}),
        New Creature("Dire Troll", "Huge", "Giant", "CE", 13, 10000, "motm 246, mtf 243", New List(Of String) From {"Arctic", "Forest"}),
        New Creature("Drow Arachnomancer", "Medium", "Humanoid (elf)", "CE", 13, 10000, "motm 99, mtf 182", New List(Of String) From {"", ""}),
        New Creature("Nalfeshnee", "Large", "Fiend (demon)", "CE", 13, 10000, "mm 62", New List(Of String) From {"", ""}),
        New Creature("Narzugon", "Medium", "Fiend (devil)", "LE", 13, 10000, "motm 190, mtf 167", New List(Of String) From {"", ""}),
        New Creature("Neothelid", "Gargantuan", "Aberration", "CE", 13, 10000, "motm 193, vgm 181", New List(Of String) From {"", ""}),
        New Creature("Rakshasa", "Medium", "Fiend", "LE", 13, 10000, "mm 257", New List(Of String) From {"", ""}),
        New Creature("Star Spawn Seer", "Medium", "Aberration", "NE", 13, 10000, "motm 230, mtf 236", New List(Of String) From {"", ""}),
        New Creature("Storm Giant", "Huge", "Giant", "CG", 13, 10000, "mm 156", New List(Of String) From {"Coastal", ""}),
        New Creature("Ultroloth", "Medium", "Fiend (yugoloth)", "NE", 13, 10000, "mm 314", New List(Of String) From {"", ""}),
        New Creature("Vampire", "Medium", "Undead (shapechanger)", "LE", 13, 10000, "mm 297", New List(Of String) From {"", ""}),
        New Creature("Wastrilith", "Large", "Fiend (demon)", "CE", 13, 10000, "motm 258, mtf 139", New List(Of String) From {"Coastal", ""}),
        New Creature("Young Red Shadow Dragon", "Large", "Dragon", "CE", 13, 10000, "mm 85", New List(Of String) From {"", ""}),
        New Creature("Adult Black Dragon", "Huge", "Dragon", "CE", 14, 11500, "mm 88", New List(Of String) From {"", ""}),
        New Creature("Adult Copper Dragon", "Huge", "Dragon", "CG", 14, 11500, "mm 111", New List(Of String) From {"", ""}),
        New Creature("Cadaver Collector", "Large", "Contruct", "LE", 14, 11500, "motm 68, mtf 122", New List(Of String) From {"Grassland", ""}),
        New Creature("Death Tyrant", "Large", "Undead", "LE", 14, 11500, "mm 29", New List(Of String) From {"", ""}),
        New Creature("Drow Inquisitor", "Medium", "Humanoid (elf)", "NE", 14, 11500, "motm 102, mtf 184", New List(Of String) From {"", ""}),
        New Creature("Elder Brain", "Large", "Aberration", "LE", 14, 11500, "motm 120, vgm 174", New List(Of String) From {"", ""}),
        New Creature("Fire Giant Dreadnought", "Huge", "Giant (fire giant)", "LE", 14, 11500, "motm 124, vgm 147", New List(Of String) From {"", ""}),
        New Creature("Githyanki Supreme Commander", "Medium", "Humanoid (gith)", "LE", 14, 11500, "motm 141, mtf 206", New List(Of String) From {"Desert", ""}),
        New Creature("Ice Devil", "Large", "Fiend (devil)", "LE", 14, 11500, "mm 75", New List(Of String) From {"", ""}),
        New Creature("Retriever", "Large", "Contruct", "LE", 14, 11500, "motm 209, mtf 222", New List(Of String) From {"Desert", "Forest"}),
        New Creature("Adult Bronze Dragon", "Huge", "Dragon", "LG", 15, 13000, "mm 108", New List(Of String) From {"Coastal", ""}),
        New Creature("Adult Green Dragon", "Huge", "Dragon", "LE", 15, 13000, "mm 94", New List(Of String) From {"Forest", ""}),
        New Creature("Green Abishai", "Medium", "Fiend (devil)", "LE", 15, 13000, "motm 40, mtf 162", New List(Of String) From {"", ""}),
        New Creature("Mummy Lord", "Medium", "Undead", "LE", 15, 13000, "mm 229", New List(Of String) From {"Desert", ""}),
        New Creature("Nabassu", "Medium", "Fiend (demon)", "CE", 15, 13000, "motm 188, mtf 135", New List(Of String) From {"", ""}),
        New Creature("Purple Worm", "Gargantuan", "Monstrosity", "Unaligned", 15, 13000, "mm 255", New List(Of String) From {"Desert", ""}),
        New Creature("Skull Lord", "Medium", "Undead", "LE", 15, 13000, "motm 220, mtf 230", New List(Of String) From {"Desert", ""}),
        New Creature("Adult Blue Dragon", "Huge", "Dragon", "LE", 16, 15000, "mm 91", New List(Of String) From {"Coastal", "Desert"}),
        New Creature("Adult Silver Dragon", "Huge", "Dragon", "LG", 16, 15000, "mm 117", New List(Of String) From {"", ""}),
        New Creature("Githzerai Anarch", "Medium", "Humanoid (gith)", "LN", 16, 15000, "motm 142, mtf 207", New List(Of String) From {"", ""}),
        New Creature("Hellfire Engine", "Huge", "Construct", "LE", 16, 15000, "motm 152, mtf 165", New List(Of String) From {"", ""}),
        New Creature("Iron Golem", "Large", "Contruct", "Unaligned", 16, 15000, "mm 170", New List(Of String) From {"", ""}),
        New Creature("Marilith", "Large", "Fiend (demon)", "CE", 16, 15000, "mm 61", New List(Of String) From {"", ""}),
        New Creature("Phoenix", "Gargantuan", "Elemental", "N", 16, 15000, "motm 206, mtf 199", New List(Of String) From {"Desert", ""}),
        New Creature("Planetar", "Large", "Celestial", "LG", 16, 15000, "mm 17", New List(Of String) From {"", ""}),
        New Creature("Star Spawn Larva Mage", "Medium", "Aberration", "CE", 16, 15000, "motm 228, mtf 235", New List(Of String) From {"", ""}),
        New Creature("Steel Predator", "Large", "Construct", "LE", 16, 15000, "motm 232, mtf 239", New List(Of String) From {"", ""}),
        New Creature("Storm Giant Quintessent", "Huge", "Giant (storm giant)", "CG", 16, 15000, "motm 235, vgm 151", New List(Of String) From {"Arctic", "Coastal", "Desert"}),
        New Creature("Adult Blue Dracolich", "Huge", "Undead", "LE", 17, 18000, "mm 84", New List(Of String) From {"Desert", ""}),
        New Creature("Adult Gold Dragon", "Huge", "Dragon", "LG", 17, 18000, "mm 114", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Adult Red Dragon", "Huge", "Dragon", "CE", 17, 18000, "mm 98", New List(Of String) From {"", ""}),
        New Creature("Androsphinx", "Large", "Monstrosity", "LN", 17, 18000, "mm 281", New List(Of String) From {"Desert", ""}),
        New Creature("Blue Abishai", "Medium", "Fiend (devil)", "LE", 17, 18000, "motm 39, mtf 161", New List(Of String) From {"Coastal", ""}),
        New Creature("Death Knight", "Medium", "Undead", "CE", 17, 18000, "mm 47", New List(Of String) From {"", ""}),
        New Creature("Dragon Turtle", "Gargantuan", "Dragon", "N", 17, 18000, "mm 119", New List(Of String) From {"Coastal", ""}),
        New Creature("Goristro", "Huge", "Fiend (demon)", "CE", 17, 18000, "mm 59", New List(Of String) From {"", ""}),
        New Creature("Nagpa", "Medium", "Humanoid (nagpa)", "NE", 17, 18000, "motm 189, mtf 215", New List(Of String) From {"Coastal", "Desert", "Forest"}),
        New Creature("Amnizu", "Medium", "Fiend (devil)", "LE", 18, 20000, "motm 46, mtf 164", New List(Of String) From {"", ""}),
        New Creature("Demilich", "Tiny", "Undead", "NE", 18, 20000, "mm 48", New List(Of String) From {"", ""}),
        New Creature("Drow Favored Consort", "Medium", "Humanoid (elf)", "NE", 18, 20000, "motm 100, mtf 183", New List(Of String) From {"", ""}),
        New Creature("Sibriex", "Huge", "Fiend (demon)", "CE", 18, 20000, "motm 217, mtf 137", New List(Of String) From {"", ""}),
        New Creature("Balor", "Huge", "Fiend (demon)", "CE", 19, 22000, "mm 55", New List(Of String) From {"", ""}),
        New Creature("Red Abishai", "Medium", "Fiend (devil)", "LE", 19, 22000, "motm 40, mtf 162", New List(Of String) From {"", ""}),
        New Creature("Ancient Brass Dragon", "Gargantuan", "Dragon", "CG", 20, 25000, "mm 104", New List(Of String) From {"Desert", ""}),
        New Creature("Ancient White Dragon", "Gargantuan", "Dragon", "CE", 20, 25000, "mm 100", New List(Of String) From {"Arctic", ""}),
        New Creature("Drow Matron Mother", "Medium", "Humanoid (elf)", "NE", 20, 25000, "motm 104, mtf 186", New List(Of String) From {"", ""}),
        New Creature("Leviathan", "Gargantuan", "Elemental", "N", 20, 25000, "motm 171, mtf 198", New List(Of String) From {"Coastal", ""}),
        New Creature("Nightwalker", "Huge", "Undead", "CE", 20, 25000, "motm 194, mtf 216", New List(Of String) From {"Arctic", "Desert"}),
        New Creature("Pit Fiend", "Large", "Fiend (devil)", "LE", 20, 25000, "mm 77", New List(Of String) From {"", ""}),
        New Creature("Ancient Black Dragon", "Gargantuan", "Dragon", "CE", 21, 33000, "mm 87", New List(Of String) From {"", ""}),
        New Creature("Ancient Copper Dragon", "Gargantuan", "Dragon", "CG", 21, 33000, "mm 110", New List(Of String) From {"", ""}),
        New Creature("Astral Dreadnought", "Gargantuan", "Monstrosity (titan)", "Unaligned", 21, 33000, "motm 51, mtf 117", New List(Of String) From {"", ""}),
        New Creature("Lich", "Medium", "Undead", "Any evil", 21, 33000, "mm 202", New List(Of String) From {"", ""}),
        New Creature("Molydeus", "Huge", "Fiend (demon)", "CE", 21, 33000, "motm 184, mtf 134", New List(Of String) From {"", ""}),
        New Creature("Solar", "Large", "Celestial", "LG", 21, 33000, "mm 18", New List(Of String) From {"", ""}),
        New Creature("Ancient Bronze Dragon", "Gargantuan", "Dragon", "LG", 22, 41000, "mm 107", New List(Of String) From {"Coastal", ""}),
        New Creature("Ancient Green Dragon", "Gargantuan", "Dragon", "LE", 22, 41000, "mm 93", New List(Of String) From {"Forest", ""}),
        New Creature("Zaratan", "Gargantuan", "Elemental", "N", 22, 41000, "motm 278, mtf 201", New List(Of String) From {"Desert", "Forest", "Grassland"}),
        New Creature("Ancient Blue Dragon", "Gargantuan", "Dragon", "LE", 23, 50000, "mm 90", New List(Of String) From {"Coastal", "Desert"}),
        New Creature("Ancient Silver Dragon", "Gargantuan", "Dragon", "LG", 23, 50000, "mm 116", New List(Of String) From {"", ""}),
        New Creature("Elder Tempest", "Gargantuan", "Elemental", "N", 23, 50000, "motm 121, mtf 200", New List(Of String) From {"Arctic", "Coastal", "Grassland"}),
        New Creature("Empyrean", "Huge", "Celestial (titan)", "CG or NE", 23, 50000, "mm 130", New List(Of String) From {"", ""}),
        New Creature("Kraken", "Gargantuan", "Monstrosity (titan)", "CE", 23, 50000, "mm 197", New List(Of String) From {"", ""}),
        New Creature("Ancient Gold Dragon", "Gargantuan", "Dragon", "LG", 24, 62000, "mm 113", New List(Of String) From {"Forest", "Grassland"}),
        New Creature("Ancient Red Dragon", "Gargantuan", "Dragon", "CE", 24, 62000, "mm 97", New List(Of String) From {"", ""}),
        New Creature("Marut", "Large", "Construct (inevitable)", "LN", 25, 75000, "motm 173, mtf 213", New List(Of String) From {"", ""}),
        New Creature("Tarrasque", "Gargantuan", "Monstrosity (titan)", "Unaligned", 30, 155000, "mm 286", New List(Of String) From {"", ""})
        }
End Module
